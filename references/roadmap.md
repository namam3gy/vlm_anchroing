...
| **E6 Confidence-gated cross-check (§6.2 alignment, 2026-05-12)** | Post-hoc selective application of L=26 K=8 α=1.0 only when base-pass `cross_entropy` > per-dataset B2/B3 boundary. 5-dataset paired mean: Δdf(a) −2.86 → −2.52 pp (per-dataset 2/5 *strengthened* under gating, 3/5 weakened to 71–92 %), Δem(b) +8.83 → +7.28 pp (5/5 weakened, 75–94 % preserved per dataset) with projection on 66.6 % of inputs. Confirmatory evidence that the *anchoring* component of the projection localises to §6.2 B3–B6 regime while the *b-arm error-mode* component is partially regime-independent. **Not** a deployment recommendation (forces sequential 2-stage inference). `scripts/build_e6_entropy_gated_summary.py` → `docs/insights/_data/e6_entropy_gated_per_dataset.{csv,md}`. | ✅ shipped 2026-05-12 |
...
- **2026-05-12 (§7.4.5 confidence-gated cross-check — §6.2 alignment).** Post-hoc selective-application variant of the chosen E6 mitigation: L=26 K=8 α=1.0 projection applied only when base-pass `answer_span_cross_entropy` exceeds the per-dataset B2/B3 boundary (33.33 % percentile from §6.2 6-bin default). CPU-only join of E5/E7 baseline CE (`answer_span_cross_entropy`) to E6 chosen-cell predictions, 5-dataset paired evaluation on the same wrong-base sids as Stage 4-final. **Headline (5-dataset mean): Δdf(a) −2.86 → −2.52 pp, Δem(b) +8.83 → +7.28 pp, Δem(a) +3.87 → +3.11 pp, with projection applied on 66.6 % of inputs.** Per-dataset Δdf(a) heterogeneous: 2/5 datasets *strengthen* slightly under gating (TallyQA, InfoVQA), 3/5 weaken to 71–92 % (PlotQA, ChartQA, MathVista). Δem(b) uniformly weakened (5/5, 75–94 % preserved per dataset). Reading: the *anchoring* component of the projection is primarily localised in the §6.2 B3–B6 regime (df doesn't need B1–B2 intervention); the *b-arm error-mode* component is partially regime-independent (em(b) gain spans bins). Reported as confirmatory §6.2-alignment evidence, **not** a deployment recommendation — gating forces sequential two-stage inference. Artifacts: `scripts/build_e6_entropy_gated_summary.py`, `docs/insights/_data/e6_entropy_gated_per_dataset.{csv,md}`. Paper edits: `docs/paper/sections/06_confidence.md` §6.2 forward-pointer + `docs/paper/sections/07_mechanism_mitigation.md` new "Confidence-gated application" subsection before "Position in the §7 mitigation story". PR `worktree-entropy-gated-mitigation → master` (script + roadmap; paper drafts + `_data/` CSV stay gitignored in main repo).
