...
| **E6 Confidence-gated cross-check (§6.2 alignment, 2026-05-12)** | Post-hoc selective application of L=26 K=8 α=1.0 only when base-pass `cross_entropy` > per-dataset B2/B3 boundary. **Pooled (`N_pool = 8121`)**: Δdf(a) −1.91 → −1.81 pp (95 % preserved on 66.6 % of inputs), Δadopt(a) 96 %, Δem(b) 84 %, Δacc(b) 86 %, Δem(a) / Δacc(a) 77 %. Anchoring-side metrics nearly preserved → §6.2 localisation confirmed; em / acc on a- / b-arms partially preserved → (a − m) projection co-aligns a wrong-base error mode that is partially regime-independent. PlotQA Δacc(a) −1.78 pp (5 % tol.) vs Δem(a) +2.43 pp — projection moves "close-but-not-exact" → "exact" but a smaller fraction crosses 5 % band; other 4/5 datasets sign-aligned. Unconditional dominates gated on every pooled metric. **Not** a deployment recommendation (forces sequential 2-stage inference). `scripts/build_e6_entropy_gated_summary.py` → `docs/insights/_data/e6_entropy_gated_per_dataset.{csv,md}` (now em + acc + df + adopt, pooled + per-dataset). | ✅ shipped 2026-05-12 |
...
- **2026-05-12 (§7.4.5 confidence-gated cross-check — §6.2 alignment).** Post-hoc selective-application variant of the chosen E6 mitigation: L=26 K=8 α=1.0 projection applied only when base-pass `answer_span_cross_entropy` exceeds the per-dataset B2/B3 boundary (33.33 % percentile from §6.2 6-bin default). CPU-only join of E5/E7 baseline CE (`answer_span_cross_entropy`) to E6 chosen-cell predictions; pooled paired evaluation on `N_pool = 8121` wrong-base sids across the 5 datasets (TallyQA dominates ~61 %). Now also reports `acc` (dataset-official: PlotQA / ChartQA 5 % relative, InfoVQA ANLS ≥ 0.5, TallyQA / MathVista exact int = em) alongside strict em. **Headline (pooled): Δdf(a) −1.91 → −1.81 pp (95 % preserved on 66.6 % of inputs), Δadopt(a) −2.06 → −1.97 pp (96 %), Δem(b) +10.70 → +8.98 pp (84 %), Δacc(b) +9.46 → +8.10 pp (86 %), Δem(a) +5.07 → +3.89 pp (77 %), Δacc(a) +3.88 → +2.99 pp (77 %).** Reading: anchoring-side metrics (`df`, `adopt`) are almost fully preserved under gating, confirming the §6.2 localisation; em / acc on a- and b-arms are partially preserved (77–86 %), diagnostic that the (a − m) projection direction co-aligns a wrong-base error mode beyond pure anchor pull, partially regime-independent. Strict-em / dataset-acc split also exposed: PlotQA Δacc(a) = −1.78 pp under 5 % tolerance while Δem(a) = +2.43 pp — projection raises strict em but pushes a smaller fraction out of the 5 % acc band on the other side; other 4/5 datasets sign-aligned. On every pooled metric unconditional dominates gated — gated is **not** a free improvement, only a confirmation that the mechanism in §6.2 and the projection in §7.4.5 act in the same confidence regime. Reported as confirmatory §6.2-alignment evidence, **not** a deployment recommendation — gating forces sequential two-stage inference. Artifacts: `scripts/build_e6_entropy_gated_summary.py` (now emits pooled + per-dataset, em + acc + df + adopt), `docs/insights/_data/e6_entropy_gated_per_dataset.{csv,md}`. Paper edits: `docs/paper/sections/06_confidence.md` §6.2 forward-pointer + `docs/paper/sections/07_mechanism_mitigation.md` new "Confidence-gated application" subsection before "Position in the §7 mitigation story". PR #38 `worktree-entropy-gated-mitigation → master` (script + roadmap; paper drafts + `_data/` CSV stay gitignored in main repo).
...
- **2026-05-11 ~AM (Paper §4.2 digit-pixel causality two-slice
  ...
  TallyQA (E5c + E5e backfill) + 4 datasets × OneVision (E5b) — six
  rows from three different panel definitions. New Table 3 = two
  internally-uniform orthogonal slices: (A) PlotQA × 6-model E7 panel
  (6/6 (a−m) > 0, range +1.0 ~ +12.8 pp), (B) LLaVA-OneVision *(Main)*
  × 5-dataset E5b/E5e panel (5/5 (a−m) > 0, range +0.7 ~ +6.6 pp).
  PlotQA appears in both slices, +6.1 pp (E7) / +6.2 pp (E5b) cross-run
  replication. VQAv2 rows (and the missing-OneVision plan footnote)
  deleted entirely. New figure `paper_4_2_digit_pixel_causality.png`
  (2-panel) generated by `scripts/_build_figure_4_2_digit_pixel.py` —
  replaces legacy `E5c_anchor_vs_masked_adopt.png` (VQAv2 LLaVA-Interleave).
  Forward-references softened: §4.1 line 118 + §C.1 line 666 "PlotQA
  가장 큰 (a-m) gap" → "(a-m) gap top-tier (sample-size-robust)" since
  Slice B point estimates put MathVista (+6.6, n_wb=152) marginally
  above PlotQA (+6.2, n_wb=2107). §E.4 trivial-recovery confound demo
  re-anchored on Slice A + Slice B representative cells. Insight 2
  forward-link to §6.2.3 Δdf ordering (PlotQA largest / TallyQA floor)
  retained — verified consistent with new Slice B (a-m) tier structure.
  Zero new inference compute — all numbers from existing CSVs.
