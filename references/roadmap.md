...
- **2026-05-11 evening 2 (§4.6 14/84 post-hoc → prereg primary-cell reframe).** Paper §4.6 (γ-β residual-stream bridge) reframed from "14 / 84 Bonferroni-corrected cells layer-routed sign-reversal confirmed" headline into **pre-registered K = 8 mean cell family (L=33 primary + L∈{29,30,34} robustness, k=4 family)** per spec [`docs/superpowers/specs/2026-05-10-p0-1-gamma-beta-bridge-design.md`](../docs/superpowers/specs/2026-05-10-p0-1-gamma-beta-bridge-design.md) §2.3 lines 82–86 (commit `e173618`). Primary cell within-Thinking single-cell 95 % CI = [−0.317, +0.424] (point estimate +0.057) — **null at the spec-prereg cell**; k=84 reference Bonferroni CI 4/4 K=8 mean cells include 0 (Bonferroni-4 monotone follow). Outcome class = spec line 86 *"failure to transfer가 informative result"* — K=8 OneVision prior cross-architecture layer-proportional transfer 가정이 falsified, framework's implicit *universal-K sweet spot* 가정도 함께 falsified. The 84-cell L×K sweep + K=1 late-stack positive + mid-stack K=1/2/4/8 negative directional sign-reversal + (L=30, K=2, max +0.866) "argmax over 84" cell are *all* re-classified as **exploratory / hypothesis-generating**, not confirmatory; Table 5 row 1 explicitly relabeled with "argmax over 84" note. §5.4 framework "two empirical anchors" → "single confirmatory anchor (§6.4 LEACE rank-1 cross-dataset failure prediction verification) + §4.6 prereg falsification leg + exploratory directional pattern". §8.4 item 8 reconfigured: 8(a) was "free recompute" → now "**Cross-architecture confirmatory pre-registered K-sweep** on another backbone (Qwen2.5-VL / FastVLM / SigLIP-Gemma) at ~3 H100-hour per backbone", 8(b) unchanged (~5 H100-hour large-n dataset inference for §6.2.3 540-family). Cross-references propagated to abstract, §1.3, §1.5 contributions paragraph, §1.5 auxiliary-observation footnote, §8.2 limitation. No numbers changed — same canonical CSV `docs/insights/_data/gamma_beta_bridge_lk_sweep.csv` — pure framing refactor that resolves the post-hoc-selection critique by surfacing the spec-prereg surface that was previously implicit. Branch `worktree-paper-section4-6-prereg-cell`.
