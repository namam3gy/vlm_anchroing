...
| **E5c** | **Anchor-mask control.** anchor 이미지에서 *digit pixel region 만* content-preserving mask 처리 (Gaussian blur 또는 solid fill, digit의 bounding box 만 — 이미지 전체 아님). 그 masked variants 로 E5b stratified pipeline 재실행. digit-visible baseline과 비교해서 anchoring의 *digit-specific* 기여를 일반적인 두-이미지 distraction baseline에서 분리. 구현: `scripts/generate_irrelevant_number_images.py` (또는 신규 script) 에 `--mask-mode {bbox-blur, bbox-fill}` flag 추가; E5b stratified driver를 새 `inputs/irrelevant_number_masked/` directory와 함께 재사용. | llava-interleave-7b 기준 ~50 min wall (E5b와 같은 shape). 산출: digit-effect attribution figure — masked vs visible variant 간 adopt_cond delta per stratum. E5b의 wrong-base × S2 peak이 digit 자체 때문인지 2-이미지 distraction 때문인지 인과 확인. | ☐ |
...
- **2026-04-27** — **E5b 결과 land: anchoring은 uncertainty-modulated AND plausibility-windowed.** llava-interleave-7b 위에서 E5b stratified run (dataset당 n=1000 base questions, VQAv2 + TallyQA, anchor stratum 5개 × stratum당 1 anchor = question당 6 conditions). M1 paired conditional adoption (case 4 `base==a==pred` 분모 제외) 으로 측정 시 cross-modal anchoring 효과에 두 개의 compounding gate 발견:
    - **Uncertainty gate (A1 scale 검증).** `target_only` 정답 record는 anchor 효과 거의 없음 (`adopt_cond` ≤ 0.10 5 strata 모두, VQAv2 + TallyQA 둘 다). `target_only` 오답 record는 adoption magnitude 1.4–37배.
    - **Plausibility window.** wrong-base subset 위에서 adoption은 S1 [0,1] peak 후 distance 따라 급격 감쇠: VQAv2 0.130 → 0.032 → 0.010 → 0.010 → 0.003 (S1→S5); TallyQA 0.092 → 0.006 → 0.003 → 0 → 0 (S5 = wrong-base 346 record 중 정확히 zero adoption, 두 번 — implausible anchor는 *완전 거부*).
