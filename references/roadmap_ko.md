...
|---|---|---|---|---|
| **E1** | **Attention-mass 분석.** Stratified 샘플에 `output_attentions=True`. Anchor-image-token vs target-image-token vs text-token 어텐션 mass를 layer/head/condition별로 계산. | 모델당 시간 단위. 기존 7 모델 위에서 바로. | "왜 일어나는가" 리뷰어 질문에 직접 답. 가장 싼 mechanistic move. | ✅ **4 모델 + per-layer localisation 완료** (gemma4-e4b, qwen2.5-vl-7b, llava-1.5-7b, internvl3-8b; 각 n=200, 총 2,400 attention record). E1 세 claim 결정됨: (i) anchor > neutral attention **4/4**; (ii) H2 `wrong > correct` **4/4 falsified**; (iii) A7 `susceptible > resistant` **3/4** (Gemma inverts). **E1b (per-layer):** 피크 layer가 encoder family별로 다름 — SigLIP-Gemma **layer 5/42** (early, δ +0.050, 주변 음의 delta layer로 둘러싸인 knife-edge spike), Qwen-ViT **layer 22/28** (late, δ +0.015, susceptibility-gated), CLIP-ViT과 InternViT **mid-stack layer ~14–16** (δ ~+0.019). Writeup: `docs/experiments/E1-preliminary-results.md` + `docs/insights/E1b-per-layer-localisation.md`. 남은 것: ConvLLaVA + FastVLM (inputs_embeds path 확장), causal test. |
