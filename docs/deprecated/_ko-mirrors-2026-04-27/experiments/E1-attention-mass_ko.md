...
**Status:** Plan only. 구현은 E2 pilot 뒤로 queued. Source: `references/roadmap.md` §6 Tier 1 E1. *(영문 canonical: `E1-attention-mass.md`)*
...
이건 가능한 mechanistic 측정 중 가장 cheap하고 `references/project.md`가 flag한 "왜 일어나는가" 리뷰어 컴플레인에 직접 답.
...
새 스크립트 `scripts/extract_attention_mass.py`:
...
- `docs/insights/E1-attention-results.md`: per-stratum attention bar plot, per-layer trace, cross-susceptibility 비교.
