...
**Bonferroni-540 (27-cell selection layer) — why not in the headline (2026-05-11).** The 27-cell pilot grid argmax is a separate multiplicity layer beyond the 20-family used above. A "free recompute" reading 99.99 % tail quantiles of the same paired-bootstrap draws is *noninformative* on the two small-n datasets — at B = 100,000 the Bonferroni-540 Δem(b) lower bound for ChartQA n = 224 lands on `2/n = +0.89 pp` and for MathVista n = 170 on `1/n = +0.59 pp`, while parametric normal-approximation 99.99 % CIs include zero on the same two cells (LO −0.05 / −0.50 pp). The bound is "stable at the empirical floor", not at a statistical threshold. PlotQA / TallyQA / InfoVQA (n ≥ 443) agree under both methods and survive Bonferroni-540 cleanly. Headline correction therefore stays at Bonferroni-20; 27-cell selection is disclosed in §6.2.3 prose only, with §8.4 item 8(b) flagging additional large-n inference as the only way to escape the 1/n floor. Full diagnostic: `docs/insights/E6-bonferroni540-smalln-floor.md`.
