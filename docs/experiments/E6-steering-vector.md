...
Evaluated on n=5000 wrong-base subset per dataset. Same predictions.jsonl holds both baseline (no intervention) and mitigation (L26_K8_α1.0) cells. **Paired-sids comparison** (sids parseable on b+a in baseline AND mitigation arms). Auto-generated by `scripts/build_e6_stage4_summary.py`; canonical CSV at `docs/insights/_data/stage4_final_per_dataset.csv` (gitignored, regenerable).
...
- **df reduction works** (avg -2.9pp across 5 datasets).
- **adopt drops** on 4/5 datasets (mean -2.0pp; InfoVQA marginal +1pp).
- **em(a) +3.9pp benefit** (every dataset positive). Earlier "-2.4pp em(a) cost" framing was a hand-copy error from prior aggregation; corrected 2026-05-04.
- **em(b) +8.8pp recovery**: wrong-base sids on target_only get *recovered* (correctly answered) under L26_K8_α1.0 intervention even with no anchor present. Paper-novel side-effect.
- **Strict free-lunch on the wrong-base subset**: anchor pull down, exact-match up on both anchored AND non-anchored arms. Paper §7.4 needs re-framing to surface this (task #38).
...
- Δ-table generator: `scripts/build_e6_stage4_summary.py` → `docs/insights/_data/stage4_final_per_dataset.{csv,md}`
- Per-cell aggregator: `scripts/analyze_e6_pilot_cells.py`
