...
> **2026-05-10 status update.** Phase 5 R4 follow-ups (P1-3 paired-bootstrap CI + P1-6 27-cell heatmap aggregation) shipped on top of Phase 1 P0 v3. Stage 4-final table now carries 95 % CI + Bonferroni-20 (99.75 %) corrected bands; pilot grid 4-metric heatmap surfaced in §A.5.
>
...
## §A. Phase 1 P0 v3 final mitigation (2026-05-04, CI-augmented 2026-05-10)
...
27 cells: L ∈ {25, 26, 27} × K ∈ {2, 4, 8} × α ∈ {0.5, 1.0, 2.0}, on PlotQA + InfoVQA pooled wrong-base subset. **Two aggregators are available:**

1. `scripts/analyze_e6_pilot_cells.py` — original 2-metric (Δdf(a), Δem(a)) aggregator used for selection at choose-time.
2. `scripts/aggregate_e6_pilot_grid.py` (added 2026-05-10 for P1-6) — full 4-metric (Δadopt(a) / Δdf(a) / Δem(a) / Δem(b)) × 2-calibration-dataset aggregation + per-calib heatmap renderer + selection-rule replay. Outputs:
   - `docs/insights/_data/E6_pilot_grid_27cells.csv` (54 rows = 27 cells × 2 calib pilots)
   - `docs/insights/_data/E6_pilot_grid_27cells_selection_replay.md`
   - `docs/figures/E6_pilot_grid_{plotqa,infographicvqa}_heatmap.png` (4 metric × 3 layer × K-α 9-cell heatmap; chosen cell starred)

Selection rule: reject cells with Δem(a) ≤ −0.06 on either calibration dataset; rank surviving cells by combined |Δdf(a)| reduction. After fixing the wrong-base baseline filter (was averaging over correct-base too, gave −45 pp spurious dEM), L=26 K=8 α=1.0 surfaced as best surviving cell with pilot pooled mean dDF = −0.040 and dEM = +0.025.

**P1-6 selection-rule replay (2026-05-10).** **The em-deal-breaker rule is non-binding on this grid** — 0 / 27 cells rejected (PlotQA min Δem(a) = −1.2 pp on cell #19 = L=27 K=2 α=0.5; InfoVQA min Δem(a) = +0.4 pp on cell #1). The rule served as a pre-committed safety rail rather than a filter that pruned cells. Top-5 by combined |Δdf(a)|:

| rank | cell # | (L, K, α) | mean Δadopt(a) | mean Δdf(a) | mean Δem(a) | mean Δem(b) |
|---:|---:|---|---:|---:|---:|---:|
| 1 | **17** | **(26, 8, 1.0) ★ chosen** | −1.8 pp | **−4.4 pp** | +3.0 pp | +6.8 pp |
...
The chosen cell #17 ranks 1st with a 1.2 pp margin on |Δdf(a)| over runner-up #8 — *the same ex ante rule on the same pilot data does not select a different cell*. R4 CRIT-2 cherry-pick concern resolved.

### Stage 4-final eval on 5 datasets — paired-sids deltas + paired-bootstrap CI

Evaluated on n=5000 wrong-base subset per dataset. Same predictions.jsonl holds both baseline (no intervention) and mitigation (L26_K8_α1.0) cells. **Paired-sids comparison** (sids parseable on b+a in baseline AND mitigation arms). **Two generators:**

- Point estimates: `scripts/build_e6_stage4_summary.py` → `_data/stage4_final_per_dataset.{csv,md}` (gitignored, regenerable).
- **Paired-bootstrap CI (B = 10,000)** added 2026-05-10 for P1-3: `scripts/build_e6_stage4_bootstrap_ci.py` → `_data/stage4_final_per_dataset_ci.{csv,md}` + raw draws `_data/stage4_final_bootstrap_draws.npz`. Resampling unit = paired sid; per-arm `(num, den)` recomputed each bootstrap so adopt's `pb ≠ anchor` denominator and df's `pa ≠ pb` clause shift correctly per arm. 95 % equal-tail percentile + Bonferroni-corrected (5 datasets × 4 metrics = 20 family, α = 0.05/20 = 0.0025 → 99.75 %) bands on the same draws.

| Dataset | n_paired | Δ adopt(a) [95 % CI] | Δ df(a) [95 % CI] | Δ em(a) [95 % CI] | **Δ em(b) [95 % CI]** ✨ |
|---|---:|---:|---:|---:|---:|
...
Bold = 95 % CI excludes 0 in headline direction. Sign-clean count:

| Metric | 95 % CI excludes 0 | Bonferroni-20 (99.75 %) excludes 0 |
|---|:---:|:---:|
| Δ adopt(a) (− direction) | 2 / 5 | 2 / 5 |
| Δ df(a) (− direction) | 1 / 5 (PlotQA) | 1 / 5 (PlotQA) |
| Δ em(a) (+ direction) | 3 / 5 | 2 / 5 (PlotQA, TallyQA) |
| **Δ em(b)** (+ direction) | **5 / 5** | **5 / 5** |

### Verdict (CI-augmented)

- **Δ df reduction**: 5 / 5 cells point-estimate negative (range −0.3 to −5.2 pp, mean −2.9 pp), but only **PlotQA n=2,306 cell** clears 95 % CI excludes 0. Small-n cells (ChartQA n=224, MathVista n=170) have point estimates consistent with PlotQA (−4.0, −4.1 pp) but CI half-widths 5–8 pp that individually straddle 0; **InfoVQA Δdf=−0.7 pp on n=443** has 95 % CI [−4.7, +3.4] — a *zero-inclusive band* that confirms the inconclusive fence with real CI numbers. Sanity gate: half-width 0.0406 lands inside the paper's prior paired-Wilson estimate (~0.04–0.06), within ~10 % of the upper edge.
- **Δ em(b) ≈ +8.8 pp recovery is the multiplicity-robust headline.** All 5 cells are 95 % sign-clean AND remain sign-clean under Bonferroni-20 corrected (99.75 %) bands — the strict-free-lunch *Δem(non-anchored arm) ≥ 0* clause is the only metric that survives multiplicity correction on every cell. Wrong-base sids on target_only get *recovered* under L26_K8_α1.0 intervention even with no anchor present. Paper-novel side-effect.
- **Δ em(a) +3.9 pp benefit**: 3 / 5 cells 95 % sign-clean (TallyQA, PlotQA, ChartQA); 2 / 5 under Bonferroni-20 (PlotQA, TallyQA). Earlier "−2.4 pp em(a) cost" framing was a hand-copy error from prior aggregation; corrected 2026-05-04.
- **Δ adopt(a)**: 2 / 5 cells 95 % sign-clean (PlotQA, ChartQA). InfoVQA point estimate +0.9 pp is *positive* but CI [−0.5, +2.5] crosses 0; not anti-mitigation evidence.
- **Strict free-lunch on the wrong-base subset, multiplicity-robust on the b-arm**: anchor pull down (PlotQA CI-strong, others CI-borderline-individually), exact-match up on both arms (b-arm 5/5 Bonferroni-robust). Paper §6.2.3 (Korean draft) / §7.4.5 (English sister-section) reframed 2026-05-10 to lead with the b-arm Bonferroni-robust headline alongside Δdf(a) PlotQA-strong cell.
...
- Pilot grid: `outputs/e6_steering/llava-onevision-qwen2-7b-ov/pilot_grid_{plotqa,infographicvqa}_n250/predictions.jsonl` (28 cells = baseline + 27 grid × 250 sids × 4 conditions per pilot)
- Δ-table generator (point estimates): `scripts/build_e6_stage4_summary.py` → `docs/insights/_data/stage4_final_per_dataset.{csv,md}`
- Δ-table generator (CI, B=10,000): `scripts/build_e6_stage4_bootstrap_ci.py` → `docs/insights/_data/stage4_final_per_dataset_ci.{csv,md}` + raw draws `_data/stage4_final_bootstrap_draws.npz`
- Pilot grid 2-metric aggregator (selection-time): `scripts/analyze_e6_pilot_cells.py` → `_data/pilot_grid_cell_selection_pooled.csv`
- Pilot grid 4-metric aggregator + heatmap (P1-6): `scripts/aggregate_e6_pilot_grid.py` → `_data/E6_pilot_grid_27cells.csv` + `_selection_replay.md` + `docs/figures/E6_pilot_grid_*_heatmap.png`
- Insight cousins: `docs/insights/E6-stage4-paired-bootstrap-ci.md` (P1-3), `docs/insights/E6-pilot-grid-aggregation.md` (P1-6)
- Reproducible notebook: `notebooks/E6_phase5_p1_3_p1_6_demo.ipynb`
