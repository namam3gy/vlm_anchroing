...
GPU 0에서 sweep 실행 중; rate ≈ 0.22 sample-instances/sec; 시작 시점부터 ETA ~15 min.
초기 partial 테이블 (n=26, 모두 `target_only` / shift 없음 케이스)에서 hook이
target_only에서 no-op임이 확인됨 (em_target_only=0.731, 모든 strength에서 동일).
