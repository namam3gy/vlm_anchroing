...
- **n=200 CI는 넓다.** Sweep 스케일에서 부트스트랩 CI가 strength들 사이에서 크게 겹친다
  ...
  헤드라인 수치를 carrying.
- **InternVL3 prose-leak parse 손실.** InternVL3의 자유 prose ("based on…")가
  `parsed_number`로 누설되어 non-numeric string ("based")으로 들어감. 분석단의 `_to_int`이
  이를 drop. ~30 %의 record가 valid-triplet에서 탈락, n ∈ [112, 137] (200 대신). 비교는
  내부 일관 (베이스라인·treated 같은 노이즈 floor 공유), 절대 n만 작음. Phase 2 풀 스케일
  + parse-rescue 패치 (regex `extract_first_number`, 이미 `vlm_anchor.utils`에 존재)로 해결.
  여기 보고된 CI는 작은 n 기준 정직한 값.
