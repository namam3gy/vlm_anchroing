...
- **Single-model claim scope** — results from llava-interleave-7b alone do not establish that the distance dependence generalises. The roadmap update will explicitly list "extension to mid-stack cluster (LLaVA-1.5 / ConvLLaVA)" as the natural follow-up; cost ≈ 2 × 50 min ≈ 1.7 h sequential.
