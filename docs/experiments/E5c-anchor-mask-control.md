...
- **Single model.** All numbers are from `llava-interleave-7b`. The 10-model E5c extension (mid-stack cluster + Gemma + Qwen + FastVLM) is queued and would tighten the digit-pixel-causality claim across architectures. For now, this is a single-model finding with strong cross-dataset replication on that one model.
