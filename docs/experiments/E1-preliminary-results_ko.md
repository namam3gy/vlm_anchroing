...
- ~~ConvLLaVA (ConvNeXt)나 FastVLM (FastViT)으로 generalise.~~ **2026-04-24 업데이트:** extraction script에 `inputs_embeds` / -200-expand path 추가; 두 모델 모두 6-model panel에 편입 (`docs/experiments/E1b-per-layer-localisation.md` 참조). ConvLLaVA는 CLIP-ViT mid-stack 프로파일 거의 그대로 replicate; FastVLM은 새 원형 (late + text-stealing + large + 최강 A7 gating).
...
**2026-04-24 업데이트:** ConvLLaVA와 FastVLM 통합 완료 (각 n=200; FastVLM은 prose hallucination으로 answer step에서 n=75). 6-model picture는 `docs/experiments/E1b-per-layer-localisation.md`에 정리. Per-family 패턴이 4 원형으로 revise됨; E4 design 착수 가능.

## E4 전 답해야 할 open question

1. ~~Per-family 패턴이 ConvLLaVA / FastVLM에서 holds?~~ **2026-04-24 closed.** `docs/experiments/E1b-per-layer-localisation.md` 참조. ConvLLaVA = mid-stack cluster 멤버; FastVLM = 새 원형 (late + text-stealing + 큼).
