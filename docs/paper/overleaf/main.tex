...
\paragraph{Notation.} For each sample $i$, let $gt_i$ denote the ground truth, $z_i$ the anchor digit value, and $p_i^c$ the parsed prediction under condition $c \in \{b, a, m, d\}$.
...
\begin{table}[t]
  ...
  \begin{tabular}{p{2cm} p{3.4cm} p{1.8cm}}
    ...
    \midrule
    Adopt$_c$ & $P[p_i^c = z_i \mid p_i^b \neq z_i]$ & Literal copy rate. Headline of the graded-vs-categorical contrast in \S4.1. \\
    DF$_c$ & $P[(p_i^c - p_i^b)(z_i - p_i^b) > \epsilon \mid |z_i - p_i^b| > \epsilon]$ & Primary anchoring metric (sign-based, gt-free). Headline of \S4 and target of the \S6 mitigation. \\
    EM$_c$ & $\#(p_i^c = gt_i) / \#(\text{numeric pair})$ & Capability axis. The b-arm is baseline accuracy; the a-arm is anchored accuracy. \\
    \bottomrule
  \end{tabular}
...
\begin{figure}[t]
  ...
  \begin{tabular}{@{}lcc@{}}
    ...
    m & \includegraphics[width=0.20\linewidth]{figures/demo_sample_01/target.png}\,\includegraphics[width=0.20\linewidth]{figures/demo_sample_01/masked.png} & \textbf{3} $\checkmark$ (recovered) \\
