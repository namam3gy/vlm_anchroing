...
Calibration dataset 위에서 모델별 단일 peak layer가 식별되지만 single-layer ablation은 5-model 메커니즘 panel에서 5/5 null — signal은 multi-layer redundant이다 (OneVision Main 5-dataset 확장에서도 single-layer 5/5 null로 *확장 검증*; §5.2 / §5.3). 본 논문의 **routing vs integration 통합 설명 framework** 는 §5.2 multi-layer redundancy + §5.3 OneVision dataset-dependent peak 을 단일 mechanism narrative 로 묶는 *사후 synthesis* (§5.4) 이며 — multi-layer redundancy는 *attention pathway routing*의 속성이고, 그 routing의 결과가 *late residual stream에 통합*되어 single-layer site에서 접근 가능해진다는 view — framework 의 두 핵심 prediction (P2 single-direction failure + P3 late-layer integration site) 은 §6.2.4 P4 layer sweep + K=1 falsification (OneVision Main, PlotQA n=2,306 K=8 L=20-26 sig plateau + TallyQA K=1 +1.4 pp sig backfire) 과 §6.4 OneVision Main LEACE rank-1 (TallyQA borderline backfire + ChartQA mild mit / null) 의 두 method 위에서 동일 OneVision-internal scope 로 직접 verify 된다. Framework 의 layer-routing 방향성 prediction 은 별도로 §4.6 γ-β residual-stream bridge 에서 *partially prospective* leg 을 받는다 — Qwen3-VL self-calibration K=1 subspace 위 within-Thinking paired Δ 가 late-stack (L=29-34) positive + mid-stack (L=20) negative sign-reversal 로 layer-routing *방향성* 예측 확인 (K=1 cell); 단 framework의 implicit *universal K=8* 가정은 동일 L=33에서 K=1 vs K=8 9× bridge ratio로 partial falsify (배포 K=8은 §6.2.2 OneVision K ∈ {2, 4, 8} grid 위 empirical sweet spot, framework prior 아님; §4.6). **E4** upper-half attention re-weighting은 mid-stack cluster (LLaVA-1.5 / ConvLLaVA) 2 모델에서 df −9.6 ~ −14.6 % 감소 + em +0.77 ~ +1.30 pp 상승의 free-lunch를 보이지만 inference 시 anchor token span을 요구한다. **E6** subspace projection은 Main 모델의 L=26에서 K=8 SVD subspace를 PlotQA + InfoVQA pooled 1회 보정 후 inference 시 보편 적용 — 5/5 dataset에서 df 감소 (평균 −2.9 pp, PlotQA만 CI excludes 0)와 동시에 *양 arm* em 상승 (anchored +3.9 pp, non-anchored +8.8 pp; **Δem(b) 5/5 cell × Bonferroni-corrected CI sign-clean**). 6-benchmark capability preservation 검증 (매크로 +0.41 pp, HallusionBench +2.21 pp excludes zero, POPE −0.06 pp pinned to zero)으로 free-lunch가 anchoring task 외부로 확장된다.
...
**Insight 2 (Anti-scaling).** Gemma3-4b가 PlotQA (+16.8 pp) / ChartQA (+10.6 pp) / MathVista (+8.2 pp) / TallyQA (+2.1 pp) 4개 dataset에서 *27B보다 더 끌린다* (df(a) wrong-base S1; per-cell 점추정 차). InfoVQA만 −2.6 pp로 부호 역전하여 표준 scaling이 회복된다 — 따라서 "anti-scaling이 4 / 5 dataset에서 부호 일관, InfoVQA에서만 reversal"로 정확히 표현된다. Load-bearing magnitude는 chart / plot / math 3 dataset (+8 ~ +17 pp)에 집중되며 TallyQA는 marginal (+2.1 pp). 이는 *visual reasoning capability gap → 두 번째 이미지 digit 의존*이라는 메커니즘 가설과 일치 — 작은 SigLIP encoder가 차트의 정확한 답을 읽지 못할 때 가시 digit을 단서로 더 강하게 잡는다. 단순한 "큰 모델 = 강건" 직관과 어긋나며, 데이터셋의 *visual complexity*가 모델 크기보다 robustness를 더 결정함을 시사한다.

**Insight 3 (Encoder family별 robustness ordering).** 5-dataset main matrix mean df(a) (wrong-base S1) 위 robustness 순서는 — Qwen-ViT (`qwen2.5-vl-7b` 0.146 / `qwen2.5-vl-32b` 0.163) > AnyRes-SigLIP (`llava-onevision-qwen2-7b-ov` 0.178) > SigLIP (`llava-interleave-7b` 0.226) > SigLIP-Gemma (`gemma3-27b-it` 0.260, `gemma3-4b-it` 0.330). Qwen-ViT 2 모델이 panel-min, SigLIP-Gemma 두 모델이 panel-max를 점유한다 (Figure 4 navy / gold 라인). Encoder family는 ordering의 *주요 축*이지만 단독 결정 요인은 아니다 — 동일 SigLIP-So400m encoder를 공유하는 세 모델 (OneVision · LLaVA-Interleave · Gemma3-27b)의 mean df가 0.178 ~ 0.260 범위에 분포하므로 LM backbone × routing (AnyRes 등) × calibration 조합도 동시에 작동한다. §5.3에서 보듯 동일 OneVision Main 안에서도 dataset-dependent peak shift가 관측되어 *peak 위치* 차원의 일반화는 fragile하며 (§5.3 + §D.1), 본 ordering은 5-dataset average df 위에서의 cross-model behavioral pattern으로 한정한다.
...
**Insight 2 (Single-direction mitigation 실패와의 *사후 일관성*).** Multi-layer redundancy 발견과 §6.4 의 single-direction ActAdd cross-dataset 실패 + LEACE rank-1 의 OneVision Main 위 cross-dataset inconsistency (TallyQA borderline backfire + ChartQA mild mit / 나머지 null) 는 §5.4 framework 의 *post-hoc synthesis* 와 일관된 *짝 (paired observation)* 이며 (framework-level 해석은 §5.4 가 사후 부여; timing 출처 §5.4 본문 + §8.4 참조), framework 의 P2 (single-direction failure) prediction 은 §6.2.4 P4 K=1 falsification (TallyQA K=1 +1.4 pp sig backfire + PlotQA K=1 vs K=8 4.4σ gap) + 본 §6.4 LEACE rank-1 (closed-form linear erasure 위 method-independent corroboration) 의 두 angle 에서, P3 (late-layer integration site) prediction 은 §6.2.4 P4 layer sweep (PlotQA L=20-26 sig plateau + L=27 ns) 에서 *prospective verify* 된다. Framework 의 layer-routing 방향성 prediction 은 §4.6 γ-β bridge K=1 sign-reversal 에서 별도로 partial-prospective verify 받는다.
...
§5.2 의 multi-layer redundancy 결과 (5-mech panel single-layer ablation 5/5 null + upper-half multi-layer ablation 5/5 significant) + §5.3 의 OneVision Main dataset-dependent peak (Plot/Tally L=27, Info/VQAv2 L=14) — 이 두 mechanism finding 은 본 framework 작성 *이전* 에 관찰되었으며, 본 절은 두 결과를 *사후* 에 단일 mechanism narrative 로 묶는 **synthesis** 이다. Framework 의 두 핵심 prediction (P2 single-direction failure + P3 late-layer integration site) 은 framework 작성 *이후 / 별도* 로 수행된 §6.2.4 P4 layer sweep + K=1 falsification (OneVision-internal direct verify — P3: PlotQA L=20-26 sig plateau + L=27 ns; P2: TallyQA K=1 +1.4 pp sig backfire + PlotQA K=1 vs K=8 4.4σ gap) 과 §6.4 OneVision Main LEACE rank-1 (P2 method-independent corroboration — closed-form linear erasure 위 same TallyQA-as-backfire-site / ChartQA-as-null-site cross-dataset 패턴) 의 *두 method* 위에서 *prospective verify* 된다. Framework 의 layer-routing 방향성 prediction (mid-stack negative / late-stack positive sign-reversal) 은 §4.6 γ-β residual-stream bridge K=1 sign-reversal 에서 별도로 partial-prospective verify 받는다 (출처 timing 은 §8.4 참조).
...
**P2 reading (single-direction fails — and backfires).** 두 가지 OneVision-internal evidence: (i) **PlotQA n=2,306 에서 K=1 Δdf = -0.4 pp [-1.7, +0.9] *ns*** — 동일 L=26 + α=1.0 위에서 *K만* 1로 줄이면 chosen-cell K=8 의 -5.2 pp 효과가 사라진다 (gap 4.85 pp ≈ 4.4σ, eager→SDPA baseline drift ~±1 pp 5× 압도); (ii) **TallyQA n=4,975 에서 K=1 Δdf = +1.4 pp [+0.5, +2.2] sig BACKFIRE** — K=1 은 단순 fail이 아니라 *anchor pull 을 증폭*, K=8 chosen (-0.3 ns) 과 명확한 sign-flip. 본 sign-flip 은 §6.4 LEACE rank-1 의 closed-form linear erasure 위 *same OneVision Main scope* 결과 — TallyQA Δdf @α=1.0 = +0.6 pp [95 % CI border 0] *borderline backfire* + ChartQA Δdf @α=0.5 = -2.7 pp mild mitigation + 나머지 null — 와 **같은 dataset 패턴 (TallyQA-as-backfire-site / ChartQA-as-null-site)** 으로 *method-independent corroborate* 되며, **single-direction subspace 는 dataset 마다 *다른 방향* 에 정렬되어 한 dataset 에서 보정한 direction 이 다른 dataset 에서 *역효과*를 낼 수 있다**는 §5.4 P2 의 직접 증거. *§6.4 closed-form LEACE rank-1 + 본 절 K=1 SVD subspace projection 두 method 가 모두 same OneVision Main + (a − m) calibration scope 위에서 single-direction failure 의 predict-then-verify chain 을 강화*한다.
...
**연계.** 본 P4 verification 은 §5.4 Predictions 2 + 3 의 *predict-then-verify* 연계를 강화한다 — Prediction 2 는 본 절 K=1 SVD subspace projection + §6.4 LEACE rank-1 closed-form linear erasure 의 *두 method 가 same OneVision Main + (a − m) calibration scope* 위에서 동일 TallyQA-as-backfire-site / ChartQA-as-null-site 패턴 (K=1: TallyQA +1.4 pp sig, ChartQA -2.7 pp ns; LEACE: TallyQA +0.6 pp [CI border 0], ChartQA -2.7 pp [-5.4, -0.5] sig mit at α=0.5) 으로 verified; Prediction 3 은 §6.2.3 chosen-cell 점 위치 + 본 절 layer sweep 의 *shape* 으로 verified. Source: `docs/insights/_data/p4_layer_sweep_per_cell_ci.csv` (full 7-cell × 5-dataset table), `docs/insights/_data/p4_layer_sweep_bootstrap_draws.npz` (raw B=10,000 draws), `docs/insights/_data/leace_recal_per_dataset_ci.csv` (LEACE recal 5-dataset × 3 α paired-bootstrap CI; eraser tag `plotqa_infovqa_recal`), generator `scripts/aggregate_e6_layer_sweep_p4.py` + `scripts/build_e6_leace_recal_bootstrap_ci.py`, sweep launchers `scripts/_p4_layer_sweep_K1_followup.sh` + `scripts/_leace_recal_sweep.sh`.
...
자연스러운 baseline 은 single mean-anchor direction `v[L] = mean_i D[i, L, :]`. 두 단일-방향 방법 (ActAdd + LEACE) 모두 cross-dataset *실패*: ActAdd 는 TallyQA-calibrated `v` self-test 자체가 α=1 에서 backfire; LEACE [Belrose et al., 2023] 를 *rank-1 closed-form 인스턴스* 로 OneVision Main + PlotQA + InfoVQA pooled (a − m) calibration scope (§6.2.3 chosen-cell 과 동일 model + pool + substrate; eraser tag `plotqa_infovqa_recal`, N=2,757 pooled wrong-base) 위에서 fit 하면 (LEACE framework 자체는 closed-form linear concept erasure 이며, rank-1 은 본 비교에서 사용한 default 운영 모드) cross-dataset Δdf 패턴이 inconsistent — TallyQA n=2,493 에서 α=1.0 Δdf = **+0.6 pp [+0.0, +1.2]** *borderline backfire tendency* (95 % CI 하한이 정확히 0; 점추정은 양수, 크기는 §6.2.4 K=1 +1.4 pp 절반 수준), ChartQA n=224 에서 α=0.5 Δdf = **−2.7 pp [−5.4, −0.5]** *95 % CI excludes 0 mild mitigation* + α=1.0 / α=2.0 은 CI border 0 / null, PlotQA n=2,308 + InfoVQA n=443 은 점추정 음수 (~−0.5 pp) 이나 CI 가 0 을 포함하는 null, MathVista n=170 은 완전 null. **5-dataset 위에서 부호 일관 감소가 없으며, TallyQA 가 양수 / ChartQA 가 음수로 두 dataset 이 반대 방향을 가리킨다** (paired-bootstrap B=10,000, sid-paired resample, source `docs/insights/_data/leace_recal_per_dataset_ci.csv`). 본 LEACE rank-1 결과는 §6.2.4 P4 K=1 SVD subspace projection — TallyQA K=1 Δdf = +1.4 pp [+0.5, +2.2] *sig backfire*, ChartQA K=1 −2.7 pp [−6.7, +1.3] ns, PlotQA K=1 vs K=8 4.4σ gap — 와 **같은 OneVision Main + same calibration scope 위에서 같은 dataset 패턴 (TallyQA-as-backfire-site / ChartQA-as-null-site)** 을 *method-independent* 로 보인다: closed-form LEACE 와 SVD-K=1 두 다른 single-direction method 가 동일 모델·동일 pool 위에서 동일 cross-dataset 부호 분포를 산출. Per-dataset mean-anchor direction 도 측정 가능하게 다른 곳을 가리킨다 — top-norm layer 에서 `cos(v_tally, v_chartqa) ≈ 0.47-0.62`.

**Insight 1 (Single-direction failure 와 multi-layer redundancy 의 *사후 일관성*).** 본 §6.4 의 single-direction ActAdd backfire + LEACE rank-1 의 OneVision-Main cross-dataset inconsistency (TallyQA borderline backfire / ChartQA mild mit / 나머지 null) 는 §5.2 multi-layer redundancy 결과 + §6.2.4 P4 K=1 OneVision-internal cross-dataset failure (TallyQA sig backfire + PlotQA K=1 vs K=8 4.4σ gap) 와 함께 §5.4 *routing vs integration framework* 가 통합 설명하는 *세 관찰* 이다 (§5.2 는 framework 작성 *이전* 관찰, §6.4 + §6.2.4 P4 는 framework 작성 *이후* prospective verification; 본 절은 §6.4 를 §5.2 의 prediction verification 으로 *주장하지 않으며*, framework P2 prediction 의 OneVision-internal verification 으로 cite 한다). Framework 의 사후 부여 해석은 dataset 이 다르면 signal 이 다른 layer 조합에 분산되어 single direction 이 cross-dataset alignment 를 잃는다는 것이며, 이 해석이 subspace projection 으로의 도구 선택을 (사후) 정당화한다.
...
|---|---|---|---|---|
| Single-direction ActAdd | ChartQA Δdf @α=1.0 = −1.3 pp [−4.0, +1.3] *null/mit-direction* (LEACE rank-1 −2.7 pp 절반 수준); TallyQA @α=1.0 = +0.2 pp [−0.0, +0.5] *borderline backfire* (CI 하한 0); 나머지 null. 4/5 datasets Δem(b) CI-clean positive | a-arm em 거의 불변 (ChartQA −0.5 pp ~bf 미만의 small loss) | b-arm em CI-clean gain on 4/5 (TallyQA +1.1 pp / InfoVQA +1.6 pp 등) | OneVision Main + PlotQA/InfoVQA pool 위 회복 — 원래 "+57 % backfire" 는 calibration-pool artifact (LEACE rank-1과 같은 부호); single-direction은 K=8 대비 ~50 % under-mitigate; TallyQA-as-backfire-site 패턴 method-independent |
| LEACE closed-form (rank-1) | TallyQA Δdf @α=1.0 = +0.6 pp [CI border 0] *borderline backfire*; ChartQA Δdf @α=0.5 = −2.7 pp [−5.4, −0.5] *CI-clean mild mit* + α=1.0 / α=2.0 null; 나머지 null | 불변 | b-arm em +0.5 ~ +2.7 pp CI-clean on 5/5 | OneVision Main + PlotQA/InfoVQA pool 위 회복 — 원래 "+56 % backfire" 는 calibration-pool artifact (E6 evidence); single-direction redundancy |
| Query-adaptive offset (PCA + Ridge probe) | OneVision Main + 같은 calibration pool + 정렬 (Q, D) pair 위에서 probe 가 effects 를 *flatten* — 5/5 datasets 0/5 CI-clean Δdf at central cell L_q=L_target=26 / α=1.0 (range −1.0 ~ +1.5 pp); 16-cell sweep cherry-pick 위 best는 MathVista (n=170) 1/5 만 CI-clean | 불변 / mild loss | 0/5 CI-clean gain (LEACE/ActAdd와 다른 capability axis) | Probe smoothing 이 mitigation 과 backfire 둘 다 평탄화 — 원래 "1/4 임계 미달, probe overfit" 와 다른 failure mode (Q-D alignment 보정 후 regularised probe 가 near-constant correction 으로 underfit; E7 evidence) |
| CogBias decode-time | 미재실험 — mean-direction substrate가 ActAdd와 동치, decode-time 적용만 추가; ActAdd recovery pattern을 따를 것으로 *예측* (§8.4 follow-up) | — | — | Mechanism-equivalent to ActAdd; novelty axis 는 decode-time application |
| MIA-DPO LoRA (weight space) | 미재실험 — gradient-trained LoRA on (prompt, GT, anchor) preference triple; gt-distribution training bias 는 preference-pair 구성에 intrinsic (§8.4 follow-up) | **−5.85 pp on VQAv2** (원래 측정, 유지) | 미보고 | Different mechanism class (weight-space vs activation-space); 재calibration 이 bias 를 옮길 뿐 제거하지 않음 |
| **Multi-direction subspace (이 작업, K=8)** | **−0.3 ~ −5.2 pp on 5/5** | **+2.4 ~ +6.6 pp** | **+4.7 ~ +13.8 pp** | **권장 — 4-clause 동시 충족 (이 5-baseline panel 위)** |

**Insight.** OneVision Main + PlotQA/InfoVQA pool 위 단일 model 비교에서 두 가지 failure mode 가 분리된다 — (i) 세 단일-방향 method (ActAdd / LEACE rank-1 / §6.2.4 P4 K=1 SVD) 모두 ChartQA 에서 K=8 cell (-5.2 pp) 의 ~50 % 수준 magnitude 만 달성하며 TallyQA 에서 borderline-or-significant backfire — *single-direction under-mitigation* (calibration pool 정렬 후에도); (ii) QAO probe correction 은 effects 자체를 평탄화 하여 0/5 CI-clean — *probe-smoothing flat-effect*; (iii) weight-space (MIA-DPO LoRA) 는 a-arm em −5.85 pp loss — *em side-effect + training distribution bias*. Multi-direction residual projection (K=8) 은 *이 5-baseline panel 위에서* 세 failure mode 를 모두 우회하는 유일 cell — *dataset 간 shared variance direction* (per-dataset cos(v_tally, v_chartqa) ≈ 0.47-0.62 가 single-direction 한계를 강제) + *probe regularisation 없는 직접 SVD basis* + *inference 시 weight 보존* 이 동시 충족된다. 이는 우연이 아니라 §5.2 multi-layer redundancy 와 §6.2.1 (a − m) contrast 설계의 직접 결과 *이다*.
...
*Note 2 (baseline tuning effort asymmetry).* 본 5-baseline panel은 ActAdd / LEACE rank-1 / Query-adaptive offset / CogBias / MIA-DPO 각각을 method-source 문헌의 default 운영 모드로 평가했으며, E6는 27-cell pilot grid (§6.2.2) 위 ex ante selection rule을 거쳤다 — *tuning effort가 baseline 측에 비대칭하게 작다*. 본 panel 의 fair-tuning 일부가 2026-05-12~14 에 추가로 수행되었으며 (E7 evidence — ActAdd 3-cell α sweep + QAO 16-cell sweep on PlotQA/InfoVQA aligned-pool calibration), 그 결과 (i) ActAdd 와 LEACE rank-1 은 *calibration-pool axis* 가 load-bearing 임이 cross-method confirmed 되어 회복 — 단 K=8 cell 의 ~50 % magnitude 에서 plateau (under-mitigation 은 본질적, "+57 % backfire" 가 아닌); (ii) QAO 는 16-cell α/L_q sweep 에서 cherry-pick 1/5 (MathVista) 만 CI-clean — probe smoothing 이 effects 를 평탄화하여 fair-tuning 으로도 K=8 magnitude 도달 불가; (iii) ITI multi-head empirical row 와 CogBias / MIA-DPO 재실험은 §8.4 item 4 의 follow-up 항목으로 유지. 본 round의 statement는 (i)+(ii) 의 fair-tuning 결과를 반영한 5-baseline panel 결과이다.
...
**Vision-modality bias의 deployable mitigation은 causal pathway를 confounding scene variance로부터 분리하는 paired-inpaint calibration contrast 위에 구축할 수 있으며, 본 논문은 이 design pattern을 cross-modal numerical anchoring 위에 4-clause free-lunch worked example로 instantiate한다.** 이 thesis 를 떠받치는 evidence 는 세 layer 로 쌓인다. *행동 layer (§4)* — cross-modal numerical anchoring 은 6 VLM 위에서 실재하며 **두 gate 의 conjunction + plausibility 조건** (불확실성 × digit-pixel · plausibility window) 으로 정의된다; anchoring effect 의 진짜 modulator 는 *base-prediction 의 연속 confidence gradient* (§4.4 L1 6-bin, B6-B1 +19.5-23.5 pp, ≥ 4/5 strict on 51-57/80 cells) 이고 wrong-base / correct-base 분할은 그 gradient 의 거친 binary projection (§4.1 PlotQA panel df-기준 +19.0-34.4 pp; §C.1 legacy VQAv2 adopt-기준 +6.9-19.6 pp), 그리고 anchor scene 의 digit pixel 만 inpaint 로 제거하면 효과가 generic distractor 수준으로 되돌아간다 (§4.2 (a − m) gap) — 이 마지막 axis 가 thesis 의 *paired-isolation 가정* 을 행동 측에서 충족한다. *Mechanism layer (§5)* — single-layer ablation 이 5/5 null 이고 signal 이 multi-layer redundant 이며 (§5.2 + OneVision 5-dataset 확장), §5.4 **routing vs integration 사후 synthesis** 가 §5.2 redundancy + §5.3 dataset-dependent peak 을 단일 narrative 로 묶고, §6.2.4 P4 layer sweep + K=1 falsification (P3 late-layer integration site + P2 single-direction failure 양쪽 OneVision-internal direct verify) + §6.4 OneVision Main LEACE rank-1 (P2 method-independent corroboration) 에서 두 핵심 prediction 이 *prospective verify* 되고 §4.6 γ-β residual-stream bridge 에서 layer-routing 방향성 K=1 *partially prospective* 검증을 받는다 — 이 mechanism evidence 는 design pattern 의 instantiation 에서 *single-direction 이 아닌 multi-direction subspace projection* 을 선택해야 한다는 design choice 를 정당화한다. *Worked example layer (§6 / §7)* — E6 (`llava-onevision-qwen2-7b-ov` 위 L=26, K=8 SVD on (a − m) calibration, α=1.0) 가 thesis의 *proof of construction*이다: PlotQA + InfoVQA pooled 1회 calibration 후 inference 시 anchor label 없이 보편 적용되어 5/5 cross-evaluation dataset에서 direction-follow 부호 일관 감소 + 양 arm em 상승, *anchoring effect* (Δdf < 0) PlotQA n=2,306 single-dataset CI-clean + 4 small-n cell 점추정-일관-CI-borderline, *capability-side multiplicity-robust headline* non-anchored arm **Δem(b) 5/5 cell × Bonferroni-corrected CI sign-clean**, 6 held-out benchmark *axis-conditional* free-lunch (anchoring-adjacent axis dominant positive: HallusionBench +2.21 pp excludes zero; broad-capability axis neutral-to-mildly-negative; 매크로 Δ = +0.41 pp 임계 충족). γ-β reasoning amplification (×12.7 correct-base ratio, §4.5; N=1 × N=1 existence proof) 은 *auxiliary observation*으로 thesis의 supporting evidence가 아니라 anchoring 자체의 reasoning-trace 의존성에 대한 별도 관찰이다.
...
- **Open-weight 모델만 (main 분석).** §4-§7 panel은 open-weight 6 VLM (LLaVA-OneVision, LLaVA-Interleave, Qwen2.5-VL, FastVLM, Gemma3-VL, ConvLLaVA)으로 한정. 폐쇄 모델 (GPT-4o / GPT-5.1 / Gemini-2.5 / Claude-Sonnet-4.5) 위 *direct-response* anchoring evaluation은 본 논문 범위 밖이며, **§8.5 closed-API VLM-as-judge demo**가 동일 anchor stimulus를 *judge-mediated* 형태로 5 frontier closed-API model에 transfer함으로써 부분적으로 해소 (open-weight main 결과의 deployment 필요성을 closed-API 측에서 separately verify).
...
10. **§8.5 closed-API VLM-as-judge demo 확장 (pilot follow-up).** §8.5 pilot은 5 judge × 2 dataset × n=200 의 small-scale demonstration이며 다음 4 axis가 미해결로 남는다 — (a) cross-architecture closed-API panel 확장 (현 5 judge → ~10 model: GPT-4o-mini, Gemini-1.5-pro, Gemini-2.0, Claude-Opus, Mistral-Vision 등 추가); (b) anchor = 5 ceiling-push expanded panel 위 재검 (사전 2-judge run에서 saturation으로 null이었음); (c) OpenAI generation mechanism 분리 — gpt-4o → gpt-5.1 susceptibility elimination이 RLHF / scale / instruction-following improvement 중 어느 것이 driver인지 (intermediate generation gpt-4o-2024-08-06 / gpt-4-turbo / gpt-4.1 등 ladder 위 phase-out 추적); (d) gemini-2.5-pro thinking-off 비교 — Google API policy 변경 또는 alternate inference path를 통해 reasoning toggle을 같은 model 안에서 가능하게 만들면 §8.5 Insight 2의 capability-gap confound를 직접 분리 가능. 추가로 single-dim Visual Faithfulness 외 production rubric (VLFeedback 3-dim, RLAIF multi-criterion) 위 anchor effect 일반화도 §8.5 caveat (v)에서 제기된 미해결 axis. ~5–10 K closed-API call 추가 (~$200–$500 cost) + 1-day analysis.

항목 1–2는 가장 cheap한 rigor 향상, 항목 7 (재분석 only) 및 8(a) (free recompute) 가 그 다음, 항목 9 + 8(b) (~2-5 H100-hour) 가 다음, 항목 3은 가장 큰 GPU 부담을 요구하는 generalisation 항목, 항목 10은 closed-API call cost 부담 ($200–$500 + analysis day).

### 8.5 Closed-API VLM-as-judge demo (pilot)

§4–§7 main 분석은 open-weight 6 VLM panel에 한정되며 (§8.2 첫 항목), thesis의 *deployment-stake* 측 — 동일 anchor stimulus가 production-tier closed-API VLM 위에서도 surface하는가 — 는 직접 검증되지 않은 채 §8.4 후속 항목으로 남는다. 본 절은 thesis의 *worked example* 자체를 확장하지 않고, 대신 **anchor injection이 production-deployed VLM-as-judge pipeline에 transfer되는지**를 5 frontier closed-API judge × 2 standard judge benchmark × 3 arm 의 small-scale pilot으로 demonstrate한다. RLAIF, judge-mediated evaluation, reward modeling 등 closed-API VLM-as-judge가 사용되는 production setting 에 대한 *deployment necessity* 측 evidence이며, §4–§7의 mechanism / mitigation 결과를 확장하지 않는다.

**Setup.** 두 개의 표준 judge benchmark에서 각각 n = 200 (image, prompt, response) triplet을 sample (VLFeedback `MMInstruction/VLFeedback` + VL-RewardBench `MMInstruction/VL-RewardBench`); 각 record에서 *chosen* response (VLFeedback: 4 model completion 중 GPT-4V 3-dim mean rating argmax; VL-RewardBench: 2 response 중 human_ranking argmax)를 선택. 각 judge에게 Visual Faithfulness 1–5 단일-차원 점수 (VLFeedback Silkie native rubric)를 *세 arm* 위에서 요청 — *b*: target image only, *a*: target image + anchor digit "1" (`inputs/irrelevant_number/1.png`, multi-image input), *m*: target image + 동일 anchor에서 digit pixel만 Telea-inpaint로 제거. Judge 5종 (모두 Staix gateway `gateway.letsur.ai/v1` 경유, 2026-05-13/14 access): **gpt-4o, gpt-5.1** (`reasoning_effort=minimal`), **gemini-2.5-pro** (reasoning forced, Google API policy), **gemini-2.5-flash** (`reasoning_effort=minimal`), **claude-sonnet-4-5-20250929**. Total 6,000 judge call (5 judge × 2 dataset × 3 arm × 200 sample).

**결과 (Table 10).** 5 judge × 2 dataset = 10 cell의 Δ(a−b) (mean Visual Faithfulness shift) 와 P(score=1) shift를 reports. 두 *susceptible* judge 가 surface — gpt-4o (highly susceptible)와 gemini-2.5-flash (highly susceptible) — 그리고 세 *robust* judge — gpt-5.1, gemini-2.5-pro, claude-sonnet-4-5.

| Dataset | Judge | mean b → a → m | Δ(a−b) | Δ(a−m) | P(score=1) b → a |
|---|---|---|---|---|---|
| VLFeedback | **gpt-4o** | 4.49 → 3.94 → 4.41 | **−0.55** | **−0.47** | 5% → 25% (+21pp) |
| VLFeedback | gpt-5.1 | 3.89 → 3.85 → 3.86 | −0.04 | −0.01 | 12% → 13% |
| VLFeedback | gemini-2.5-pro | 3.67 → 3.68 → 3.77 | +0.02 | −0.08 | 20% → 22% |
| VLFeedback | **gemini-2.5-flash** | 4.32 → 3.89 → 3.98 | **−0.41** | −0.07 | 14% → 26% (+12pp) |
| VLFeedback | claude-sonnet-4-5 | 3.88 → 3.83 → 3.86 | −0.05 | −0.03 | 8% → 12% |
| VL-RewardBench | **gpt-4o** | 4.61 → 3.65 → 4.27 | **−0.96** | **−0.62** | 2% → 32% (+30pp) |
| VL-RewardBench | gpt-5.1 | 3.35 → 3.40 → 3.23 | +0.05 | +0.17 | 14% → 11% |
| VL-RewardBench | gemini-2.5-pro | 2.67 → 2.62 → 2.77 | +0.04 | −0.19 | 36% → 37% |
| VL-RewardBench | **gemini-2.5-flash** | 4.35 → 3.75 → 3.79 | **−0.58** | −0.04 | 8% → 25% (+17pp) |
| VL-RewardBench | claude-sonnet-4-5 | 3.26 → 3.11 → 3.14 | −0.15 | −0.03 | 5% → 11% |

**Table 10.** Closed-API VLM-as-judge anchoring pilot, 5 judge × 2 dataset × 3 arm × n=200. Visual Faithfulness 1–5 (single-dim, VLFeedback Silkie native rubric). Δ(a−b) = mean(a) − mean(b); Δ(a−m) = mean(a) − mean(m) (digit-pixel isolation per §4.2). P(score=1) shift는 가장 가혹한 floor adoption proxy. n_pair은 chosen-response 200 sample 중 양 arm parse-success record 수 — gemini-2.5-pro VL-RewardBench는 reasoning-chain budget 초과로 165 (caveat (ii) 참조), 나머지 cell은 193–199. Bold = |Δ(a−b)| ≥ 0.4 (susceptible cell).

**Insight 1 — 두 mechanism이 (a − m) 분해로 separate된다.** §4.2 (digit-pixel causality)와 동일한 paired contrast 가 closed-API judge 위에서도 두 attack pattern을 isolate한다. **gpt-4o 는 *digit-specific*** — Δ(a−m) = −0.47 (VLFeedback) / −0.62 (VL-RewardBench) ≫ Δ(m−b) = −0.08 / −0.34, 즉 anchor digit pixel 자체가 score를 떨어뜨림 (adversarial watermark-style attack에 직접 취약). **gemini-2.5-flash 는 *distractor-general*** — Δ(a−m) ≈ 0 (−0.07 / −0.04); Δ(m−b) ≈ Δ(a−b), 즉 두 번째 image 가 추가되는 사실 자체가 score를 떨어뜨림 (multi-image instruction-following weakness, anchor digit이 load-bearing이 아님). 두 attack pattern은 다른 threat model을 정의한다 — anchor injection 방어 vs multi-image robustness 보강.

**Insight 2 — 두 within-vendor ablation이 robustness driver를 시사한다.** *OpenAI generation* — gpt-4o (susceptible) → gpt-5.1 (`reasoning_effort=minimal`, 즉 non-reasoning mode, robust): 같은 vendor 내 generation upgrade가 anchor susceptibility를 사실상 제거. Mechanism은 untested (RLHF 변화 / scale / instruction-following 개선 중 어느 것이 driver인지 분리되지 않음). *Google reasoning* — gemini-2.5-pro (reasoning ON, robust) vs gemini-2.5-flash (reasoning OFF, susceptible): 같은 vendor 내에서 reasoning toggle이 susceptibility를 dramatically 갈라놓음. "Reasoning provides anchor-resistance" 가설을 뒷받침하지만, pro-vs-flash capability gap에 의해 confound — 가장 깨끗한 causal test (gemini-2.5-pro thinking-off)는 Google API policy 상 불가. 한편 §4.5의 open-weight 측 *reasoning amplifies anchoring* 결과 (Qwen3-VL Instruct vs Thinking, ×12.7 correct-base ratio)와 **부호가 반대**라는 점을 명시 — open-weight 측은 *behavioral* (reasoning이 anchor adoption을 *증폭*) , closed-API judge 측은 *evaluative* (reasoning이 judge의 anchor *resistance* 강화). 두 결과는 mutually exclusive가 아닌, anchor-related processing이 *task setting*에 따라 amplifying 또는 resisting 양쪽으로 모두 작동할 수 있음을 시사하며, 본 pilot 단독으로 reasoning ↔ anchor의 일반 framework 주장은 내리지 않는다.

**Caveats.** (i) *Chosen-response selector는 worst-case adversarial framing이다.* 사전 ablation에서 random-response selector는 gpt-4o anchor signal을 ~10× 약화시킴; 본 pilot은 baseline-conditional anchor effect (judge가 자연스럽게 5/5 가까이 줄 baseline에서 anchor가 floor를 칠 수 있음) 의 worst case를 reports. Average-case는 attenuated. (ii) *gemini-2.5-pro VL-RewardBench parse-failure rate 10% (n_pair = 165 vs 다른 cell 199 / 200)* — VL-RewardBench가 math / MMMU-Pro / hallucination task를 포함해 reasoning chain이 길어지고 `max_output_tokens = 2048`을 초과; null result 읽기에는 충분하지만 다른 cell과 직접 비교는 주의. (iii) *Staix gateway는 alias-only model ID를 반환* — 본 paper citation은 alias + access date pattern 사용 (Claude만 explicit version `claude-sonnet-4-5-20250929`). (iv) *Anchor = 5 ceiling-push는 expanded panel 위 미재검* — 사전 2-judge run에서 anchor = 5 cell은 saturation으로 null (대부분 baseline 이미 5/5 ceiling). 본 pilot은 anchor = 1 floor-push 만 reports. (v) *Single-dim Visual Faithfulness 만 평가됨* — multi-dim CoT + Helpfulness / Ethics 등은 사전 design ablation 단계에서 시도되었으나 anchor signal을 약화시켜 채택하지 않음; production rubric (예: VLFeedback 3-dim, RLAIF 5-criterion 등) 위 anchor effect는 미검증.

**Lands**: figure `docs/figures/judge_pilot_v1_5judges_2datasets_n200.png` (5 judge × 2 dataset line overlay, n = 200). Code: `src/vlm_anchor/{vlfeedback_loader, vlrewardbench_loader, judge_clients, judge_pilot_data}.py`, `scripts/{build_judge_pilot_dataset, run_judge_pilot, analyze_judge_pilot}.py`. Configs: `configs/judge_pilot.yaml`, `configs/judge_pilot_vlrewardbench_a1.yaml`. Reproducibility — full evidence, per-arm decomposition source, caveat (i)-(v) 정량 source 는 부록 §A.5 reproducibility pointer 참조. Out-of-scope for §4–§7 main thesis; pilot demo만, mechanism / mitigation 일반화 주장 없음. 후속 작업 항목은 §8.4 item 10 참조.
...
- **§8.5 closed-API VLM-as-judge demo.** 5 judge × 2 dataset × 3 arm × n=200 의 full evidence — Table 10 underlying numbers (per-judge mean / Δ / P(score=1) on b · a · m), per-arm Δ(a−m) / Δ(m−b) decomposition, 5 caveat (i)-(v) source, paired-bootstrap CI script, Staix gateway access log + judge model alias 매핑 — source: `docs/insights/judge-pilot-results.md`. 동일 file은 chosen vs random response selector ~10× attenuation 사전 ablation 및 v2 multi-dim CoT discarded variant 의 historical context도 포함한다 (본문 §8.5 caveat (i) + (v) 의 정량 source).
