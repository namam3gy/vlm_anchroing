...
Design pattern과 worked example을 *evidence가 licensing* 한다 — (i) §4의 5 dataset × 6 model cross-dataset 위에서 anchoring을 wrong/correct binary projection이 아닌 *continuous confidence gradient*로 재해석하는 세 측정 axis 증거 (L1 6-bin gradient, (a − m) digit-pixel causality, wrong/correct binary stratification) 는 (a − m) paired-inpaint이 *실제로 인과 통로를 isolate한다*는 prerequisite을 제공하고 — digit pixel만 inpaint로 제거하면 효과가 generic distractor 수준으로 되돌아간다는 §4.2 (a − m) gap이 design pattern의 *paired-isolation 가정*을 행동 axis에서 충족; (ii) §5의 mechanism evidence — single-layer ablation이 5/5 null이고 signal이 multi-layer redundant라는 사실 + §5.4 *routing vs integration 사후 synthesis* + §6.4 single-direction cross-dataset failure에 대한 사전 예측의 *single confirmatory verification* (LEACE rank-1 ChartQA +56 % 역행) — 은 design pattern을 구현할 때 *single-direction이 아닌 multi-direction subspace projection*을 선택해야 한다는 design choice를 정당화한다; (iii) 평가 protocol 자체 — independent-anchor open-numeric-estimation 4-condition (b/a/m/d) + gt-자유 direction-follow metric — 은 VLMBias [Vo, Nguyen et al., 2025]의 familiar-subject counting paradigm과 *cue independence × measurement type* 두 축에서 상보적이며 (§2 · §3), (a − m) paired contrast를 *측정 가능하게 만드는* substrate이다. 배포된 E6의 K=8 parameterization은 OneVision K ∈ {2, 4, 8} grid 위 empirical sweet spot (§6.2.2) 이며, **§4.6 γ-β residual-stream bridge의 사전등록된 (L=33, K=8) primary cell이 single-cell 95 % CI에서 null인 것** — *K=8 OneVision prior의 cross-architecture layer-proportional transfer 가정이 prereg test에 의해 falsified* — 이 design pattern instantiation의 *operational hyperparameter*가 cross-architecture 재calibration을 요구한다는 운영적 결과를 직접 산출한다 (§8.2 + §8.4 item 3 cross-architecture E6 + §8.4 item 8 cross-architecture prereg K-sweep).

*Auxiliary observation* — Reasoning-mode Qwen3-VL-8B-Thinking이 same continuous-confidence axis 위에서 anchor pull을 amplify하는 N=1 architecture × N=1 dataset *existence proof* (§4.5 behavioural; §4.6 residual-stream bridge가 사전등록된 (L=33, K=8) primary cell에서 *informative null* — K=8 OneVision prior cross-architecture transfer falsified — 을 산출하면서 exploratory K-sweep에서 K=1 directional sign-reversal을 hypothesis-generating으로 보고) 는 design pattern의 *transferability*에 대한 부수 evidence가 아니라 *anchoring 자체의 reasoning-trace 의존성*에 대한 별도 관찰이다. §4.5 / §8.2 한계 참조.
...
**Insight 3 (PlotQA un-mitigated free-lunch).** Slice A E7 PlotQA panel의 *놀라움*: 6개 중 5개 모델이 `em(a) > em(b)` (em delta +0.6 ~ +5.0 pp; per-model em table source 부록 §A.5). S1 cutoff가 anchor를 GT의 ±10 % 안에 두므로 anchor를 "그럴듯한 추측 단서"로 픽업하는 모델은 정확도를 *얻는다*. 이 패턴은 InfoVQA로 일반화하지 *않으며* (혼합 부호 — 같은 evidence file의 InfoVQA panel; 부록 §A.5), *§6.2의 free-lunch mitigation이 이 PlotQA baseline 패턴을 5-dataset에 일반화 가능한 복구 메커니즘으로 변환하는 정확한 도메인*이다.
...
**Table 4.** γ-β H2 decomposition (Qwen3-VL-8B Instruct vs Thinking, MathVista S1 single-stratum). Bold = post-Thinking values where wrong − correct df gap collapses (Instruct +0.235 → Thinking +0.060). 비율 행은 Thinking / Instruct point estimate (CI 미산출 — Instruct correct df denominator small (n_correct ≈ 249); §8.4 item 9).
...
**Insight 1 (Continuous gradient가 진짜 modulator라는 직접 증거).** §4.4의 L1 6-bin gradient가 *통계적 인공물이 아닌 mechanism-bound*라는 가설을 강하게 뒷받침한다. anchor pull을 증폭하리라 기대되는 조작 (reasoning trace = anchor를 active 비교 후보로 더 오래 유지)이 실제로 anchor pull을 증폭하며, 그 증폭이 *binary projection을 깨는 정확한 방식*은 framework의 예측과 일치한다 — *B1+B2+B3 vs B4+B5+B6 평균 차이*가 사라지려면 (i) B4+B5+B6 (wrong-base 우세 cohort) 자체에 ceiling이 있거나 (ii) B1+B2+B3 (correct-base 우세 cohort) 가 push up되거나 둘 중 하나여야 하는데, γ-β는 (ii)에 해당 — *correct-base에서 df가 ×12.7 push up* 되어 binary 평균 차이가 축소된다. 즉 Mussweiler-Strack의 "낮은 baseline confidence → anchor가 비교 후보로 더 쉽게 활성화" 메커니즘이 reasoning-mode의 trace 연장에 의해 외부에서 *조작 가능*하며, 그 조작은 confidence axis 위에 정확히 떨어진다 — Insight 2의 LRM 문헌 정렬과 합쳐 §1.5의 auxiliary observation (reasoning-amplifies-anchoring existence proof) 의 직접 증거를 구성한다.

**Insight 2 (LRM 문헌과 정렬).** 텍스트 LRM에서 Wang et al. [2025a]이 LRM judging bias 5-class (bandwagon / authority / position / distraction / superficial-reflection) 전반에 보고한 *reasoning-amplifies* pattern — 그중 distraction-bias 계열이 본 논문 anchoring과 가장 가까운 cross-modal 유추 — 이 VLM에서 *N=1 architecture × N=1 dataset existence proof* 형태로 재현되며, *reasoning mode는 robustness 보강이 아니라 anchor 쪽 backslide 위험원*이라는 운영적 함의가 도출된다.

**Insight 3 (acc(d)도 *낮다*).** Thinking은 acc(d)도 더 *낮다* (Instruct 0.647 = 249/385, Thinking 0.587 = 226/385; γ-β 단일-stratum 설정에서 별도 b-arm run이 없어 d-arm correct fraction을 b-arm baseline proxy로 사용). Reasoning trace가 정확도 향상을 보장하지 않을 뿐 아니라 (test-time-compute inverse-scaling 보고와 일관 [Bae et al., 2025]), robustness *낮춤*까지 동시에 가져온다 — trace가 길수록 bias 누적 기회가 커진다는 *기능적 trade-off*가 이 pair의 직접 관측이다.

### 4.6 γ-β residual-stream bridge — framework의 prospective test

**Claim.** §5.4 routing vs integration framework는 §6의 OneVision integration-site finding (L=26, K=8 subspace projection이 free-lunch mitigation을 산출) 이 *cross-architecture layer-proportional transfer* 형태로 다른 backbone에 옮겨가는지를 묻는 falsifiable prediction을 발화한다. 본 절은 framework 작성 *이후* 실행된 Qwen3-VL self-calibration 실험으로 이 prediction을 사전등록된 cell 위에서 직접 검증한다.

**Setup.** Qwen3-VL-Instruct를 PlotQA + InfoVQA + TallyQA pooled (a − m) wrong-base 잔차 (n=3,017) 위에서 self-calibrate해 V_K[L]을 얻은 뒤, γ-β stimuli (a-S1 anchor + d neutral) 위에서 Qwen3-VL-{Instruct, Thinking}의 per-generated-token 잔차를 사영하고 trace 평균/최대 진폭을 측정. Within-Thinking paired statistic은 sid 단위 (T_a − T_d) 차이; paired bootstrap B = 10,000.

**Pre-registered analysis (commit `e173618`, [`docs/superpowers/specs/2026-05-10-p0-1-gamma-beta-bridge-design.md`](../superpowers/specs/2026-05-10-p0-1-gamma-beta-bridge-design.md) §2.3).** 본 실험은 사전등록된 단일 cell family를 갖는다 — *primary cell* `(L = 33, K = 8, statistic = mean, sub-group = all-base)` + *robustness sweep* at `L ∈ {29, 30, 34}` at same K = 8 mean. **L = 33의 logic**: OneVision Main의 §6 integration site L = 26 / 28-layer ≈ 0.929를 36-layer Qwen3-VL에 layer-proportional transfer하면 L = 33 / 36 ≈ 0.917. **K = 8의 logic**: §6 OneVision prior 그대로. 명시된 outcome semantics (spec line 86): "K = 8 prior + proportional L을 그대로 쓰고 *failure to transfer*가 나면 그 자체가 informative result."

**Table 5.** Pre-registered K = 8 mean cell family (4 cells) within-Thinking Δ. Bonferroni-corrected k = 4 confirmatory conclusion은 prose에서 derive — k = 84 CI는 strict superset이며 (k = 84) 위 4 prereg cell 모두 0 미포함이 0/4이라는 사실로부터 k = 4 conclusion이 monotone follow.

| layer | K | stat | n | within-Thinking Δ | 95 % CI (single-cell) | Bonferroni 99.94 % CI (k = 84 reference) | 95 % excludes 0 | Bonf-84 excludes 0 |
|---|---:|---|---:|---:|---|---|:-:|:-:|
| **33 (primary)** | 8 | mean | 522 | +0.057 | [−0.317, +0.424] | [−0.549, +0.648] | ✗ | ✗ |
| 29 (robustness) | 8 | mean | 522 | +0.418 | [+0.088, +0.748] | [−0.106, +1.006] | ✓ | ✗ |
| 30 (robustness) | 8 | mean | 522 | +0.413 | [+0.039, +0.786] | [−0.192, +1.048] | ✓ | ✗ |
| 34 (robustness) | 8 | mean | 522 | +0.828 | [−0.399, +2.036] | [−1.195, +2.869] | ✗ | ✗ |

**Pre-registered outcome — K = 8 OneVision prior transfer falsified.** Primary cell (L = 33) **null at single-cell 95 % CI** (Δ = +0.057, includes 0); robustness sweep 2/3 cells (L = 29, L = 30) single-cell 95 % CI excludes 0 marginally (lower bounds +0.088 / +0.039) but **0/4 cells survive even the k = 84 Bonferroni correction**, and since k = 84 percentile-CI is *strictly more conservative* than the prereg-family k = 4 correction, **0/4 cells survive Bonferroni-4 by monotonicity**. §6의 K = 8 / L = 26 OneVision integration-site finding은 Qwen3-VL backbone에 layer-proportional transfer되지 않는다 — spec이 예상한 "failure to transfer가 informative result" outcome class에 정확히 해당. **§5.4 framework의 *implicit universal-K = 8 sweet spot* 가정이 본 prereg test에 의해 falsified.**

**Exploratory K-sweep (hypothesis-generating, not confirmatory).** Prereg test가 K = 8 universal 가정을 falsify한 뒤 *후속 hypothesis-generating* 작업으로 K-grid를 K ∈ {1, 2, 4, 8, 12, 16} × L ∈ {14, 20, 25, 29, 30, 33, 34} × statistic ∈ {mean, max} = 84 cells로 확장하고 Bonferroni-corrected over k = 84 family. 본 sweep은 prereg framework의 일부가 아니며 *cross-architecture confirmatory pre-registered K-sweep on another VLM (e.g., Qwen2.5-VL)* 은 §8.4 item 8로 명시한다.

**Table 6.** 84-cell exploratory K-sweep within-Thinking 대표 cells (전체 84-cell table 부록 §F). *Cell #1은 84 cells 전체에 대한 triple post-hoc argmax (7 L × 6 K × 2 stat) ― confirmatory finding이 아닌 descriptive strongest-cell statistic.*

| layer | K | stat | n | within-Thinking | 95 % CI | Bonferroni 99.94 % CI (k = 84) | 라벨 |
|---|---:|---|---:|---:|---|---|---|
| 30 | 2 | max | 522 | +0.866 | [+0.412, +1.330] | [+0.115, +1.643] | **argmax over 84** |
| 30 | 1 | mean | 522 | +0.477 | [+0.254, +0.695] | [+0.082, +0.852] | late-stack K = 1 |
| 29 | 1 | mean | 522 | +0.446 | [+0.252, +0.635] | [+0.123, +0.793] | late-stack K = 1 |
| 33 | 1 | mean | 522 | +0.284 | [+0.188, +0.380] | [+0.113, +0.447] | late-stack K = 1 |
| 25 | 1 | mean | 522 | +0.213 | [+0.158, +0.270] | [+0.123, +0.314] | transitional K = 1 |
| 20 | 1 | mean | 522 | −0.152 | [−0.189, −0.116] | [−0.213, −0.094] | mid-stack K = 1 |
| 20 | 4 | mean | 522 | −0.192 | [−0.232, −0.152] | [−0.269, −0.124] | mid-stack K = 4 |
| 14 | 1 | mean | 522 | −0.041 | [−0.054, −0.028] | [−0.064, −0.020] | early-mid K = 1 |

14 / 84 cells가 Bonferroni-corrected k = 84 CI에서 0 제외. **Directional pattern (exploratory)**: late-stack (L = 29, 30, 33) K = 1 mean positive (+0.21 ~ +0.48), mid-stack (L = 20) K = 1 / 2 / 4 / 8 mean negative (−0.11 ~ −0.19), early-mid (L = 14) 매우 작은 negative — framework가 예측한 mid-stack suppress / late-stack integrate sign-reversal과 directionally consistent. 그러나 *어느 (L, K, statistic) 조합이* Bonferroni-clean 할지를 prereg에서 specify하지 않았으므로, 14/84은 framework의 directional prediction에 *consistent한 cell의 fraction*이며 confirmatory verification rate가 아니다.

**Insight 1 (Prereg test가 universal-K 가정을 falsify; K = 1 rescue는 hypothesis-generating).** 동일 L = 33 + 동일 data + K = 1 vs K = 8 비교에서 prereg primary cell (K = 8) 은 null (Δ = +0.057) 인 반면 K = 1 exploratory cell은 single-cell Bonferroni-positive (+0.284 [+0.113, +0.447] under k = 84 correction) — *9× ratio of within-Thinking magnitude*. Qwen3-VL의 sv7/sv8 elbow가 1.026 gradual decay인 사실이 K = 8 prior의 dilution을 사후 해석할 수 있는 calibration-spectrum 증거이나, **K = 1 fallback decision은 prereg test의 null outcome을 보고 *이후* 내려진 것이므로 본 paper는 K = 1 결과를 confirmatory가 아닌 *hypothesis-generating for §8.4 item 8 cross-architecture prereg K-sweep*** 로 분류한다. Framework의 falsifiability와 honest disclosure를 보장하는 핵심 element이며, cross-architecture E6 응용 시 K-sweep이 *prereg-mandated*가 되는 근거 (§8.2).

**Insight 2 (Quantitative interlock 미해결).** K = 1 exploratory cell의 within-Thinking magnitude (+0.5 ~ +0.9 amplitude units, baseline 위 ~0.2 ~ 0.4 % 상대 변화) 는 §4.5 Table 4의 correct-base df ratio 큰 폭 증가와 *정량적*으로 정렬되지 않는다 — K = 1 V_K[L=*]는 anchor 처리의 *one aspect*만 capture하며, 다른 잔차 차원·attention pathway·output-head dynamics가 behavioral gap의 대부분을 운반한다. 본 절은 prereg test의 *null primary outcome* + exploratory K-sweep의 *directional consistency* 두 register를 분리 보고하며, 본 paper의 confirmatory mechanism evidence는 §6.4 LEACE rank-1 ChartQA +56 % 역행 단일 source로 한정한다 — §5.4 framework는 "two confirmatory anchors" 가 아니라 "one confirmatory (§6.4) + one exploratory (§4.6)" 형태로 명시 (§5.4 본문). *Qualitative directional bridge exploratorily established / quantitative interlock + cell-level confirmatory test deferred* (§8.2 + §8.4 item 8).
...
### 5.4 Routing vs integration site framework — 사후 synthesis와 single confirmatory anchor

§5.2의 multi-layer redundancy 결과 (5-mech panel single-layer ablation 5/5 null + upper-half multi-layer ablation 5/5 significant) + §5.3의 OneVision Main dataset-dependent peak (Plot/Tally L=27, Info/VQAv2 L=14) + §6.4의 LEACE rank-1 ChartQA +56 % 역행 — 이 세 mechanism finding은 본 framework 작성 *이전*에 모두 관찰되었으며, 본 절은 이 세 결과를 *사후*에 단일 mechanism narrative로 묶는 **synthesis**이다. **Framework의 single confirmatory anchor는 §6.4 LEACE rank-1 cross-dataset 실패의 사전 예측 verification**이며 (§5.4 framework가 *single-direction은 multi-layer redundancy로부터 cross-dataset 실패가 예측된다*는 정식화를 산출한 뒤 §6.4가 이를 ChartQA +56 % 역행으로 verify), §4.6 γ-β residual-stream bridge가 framework 작성 이후 별도로 수행되었으나 *사전등록된 (L=33, K=8) primary cell이 informative null* — K=8 OneVision prior cross-architecture transfer 가정이 falsified — 로 framework의 implicit *universal-K* 가정을 부분 falsify하면서 exploratory K-sweep에서 directional sign-reversal을 hypothesis-generating으로 보고한다 (§4.6).
...
**Framework의 prereg falsification leg (§4.6).** §5.2 + §5.3 + §6.4 mechanism observation을 사후 통합한 synthesis 위에 §4.6 γ-β residual-stream bridge가 *사전등록된* (L=33, K=8, mean, all-base) primary cell에서 K=8 OneVision prior의 layer-proportional cross-architecture transfer 가정을 직접 test한다 (framework 작성 이후 실행, spec commit `e173618` 기준 prereg). **Primary cell single-cell 95 % CI = [−0.317, +0.424]** (point estimate +0.057) **null** — K=8 prior가 Qwen3-VL backbone에 transfer되지 않으며 framework의 implicit *universal-K sweet spot* 가정을 falsify. Robustness sweep 3 cell도 Bonferroni-4 corrected 후 0/4 cells excludes 0 (k=84 reference CI 4/4 includes 0이 monotone Bonferroni-4 conclusion을 제공; §4.6 Table 5). 후속 exploratory K-sweep (k=84 Bonferroni-corrected) 에서 K=1 late-stack positive + mid-stack negative directional sign-reversal이 14/84 cells에서 0 제외 — directionally framework-consistent이나 *prereg-confirmatory* 가 아닌 *hypothesis-generating* 분류 (§4.6 Insight 1). **Cross-architecture confirmatory pre-registered K-sweep** (예: Qwen2.5-VL 위 K-grid prereg) 은 §8.4 item 8로 정식 등록 — framework의 falsifiability를 지속적으로 보장하는 ongoing prereg 원칙이다.
...
**Table 6.** E4 Phase 2 결과, mid-stack cluster 2 모델 (LLaVA-1.5 / ConvLLaVA), 88,650 records / 모델. Δdf 상대 = (df_mit − df_base) / df_base; Δem(a)는 a-arm exact-match. acc(b)·acc(d)는 mitigation 외부 column으로 free-lunch 검증. Bold = 열 단위 가장 큰 효과. Reproducibility pointer 부록 §A.5.
...
- **γ-β residual-stream bridge: prereg primary null + exploratory directional pattern, quantitative interlock 미해결.** §4.6의 사전등록된 (L=33, K=8, mean, all-base) primary cell이 single-cell 95 % CI [−0.317, +0.424] 로 **null** — K=8 OneVision prior의 Qwen3-VL backbone layer-proportional cross-architecture transfer 가정을 *prereg test에 의해 falsified*. Robustness sweep 3 cell + k=84 reference CI도 4/4 cells includes 0 (Bonferroni-4 monotone follow). 후속 *exploratory* K-sweep (k=84) 에서 K=1 late-stack positive + mid-stack negative directional sign-reversal이 14/84 cells에서 0 제외 — anchor 처리가 layer-routed 형태로 잔차에 흔적을 남긴다는 *hypothesis-generating directional bridge*는 establish되었으나 *prereg-confirmatory가 아니다*. 또한 **§4.5의 ×12.7 correct-base behavioral df ratio는 잔차 amplitude 측정에서 정량적으로 재현되지 않았다** (exploratory K=1 magnitude +0.5~+0.9 amplitude units, baseline 위 0.2~0.4 % 상대 변화). K=1 V_K[L=*]는 anchor 처리의 *one aspect*만 capture하는 instrument이며, 다른 잔차 차원·attention pathway 차이·output-head dynamics가 ×12.7 gap의 대부분을 carry한다고 해석한다. Cross-architecture confirmatory pre-registered K-sweep + quantitative interlock 추구 (digit-bbox-restricted (a − m) calibration / attention-head 단위 ITI-style locus 변경 / output-head logit-level mediator 분석) 는 §8.4 item 8 후속 라운드 작업.
...
8. **Cross-architecture pre-registered K-sweep on §4.6 bridge (§4.6 confirmatory hardening) + §6.2.3 multiplicity hardening.** §4.6 prereg primary cell (Qwen3-VL L=33, K=8, mean) 의 *informative null* 은 framework의 universal-K 가정 falsification으로 본 paper에 ship되었으나, exploratory K-sweep에서 산출된 K=1 late-stack positive + mid-stack negative directional pattern은 hypothesis-generating으로 분류된다. 본 항목은 두 갈래 — (a) **Cross-architecture confirmatory pre-registered K-sweep**: 다른 backbone (예: Qwen2.5-VL, FastVLM, SigLIP-Gemma) 에서 self-calibrated V_K[L]을 산출한 뒤, *bridge inference 데이터 노출 전*에 K ∈ {1, 2, 4, 8} × L ∈ {layer-proportional band, mid-stack reference} cell 집합을 사전 등록하고 spec freeze. K=1 late-stack positive + mid-stack K=1/2/4 negative 두 predictions가 새 backbone에서 confirmatory 검증되면 routing-vs-integration framework의 cross-architecture generalisability를 strengthen; 한 쪽이라도 falsify되면 framework가 Qwen2 backbone-specific으로 narrow된다는 honest disclosure로 갱신. ~3 H100-hour per backbone. (b) **§6.2.3 mitigation evaluation 추가 large-n dataset 위 반복**: small-n bootstrap floor에 묶이지 않은 cell들로 27-cell selection layer를 strict하게 보정 (현재 paper의 540-family disclosure는 *prose-level* — 추가 large-n inference 없이는 ChartQA n=224, MathVista n=170의 1/n 분위수 한계를 우회할 수 없음; 부록 §A.5 참조). ~5 H100-hour (PlotQA · TallyQA 외 추가 n ≥ 1,000 dataset 1-2개에서 chosen cell #17 inference). (a) + (b) 모두 본 paper의 exploratory/directional findings를 confirmatory level로 hardening하며, framework의 falsifiability를 *ongoing prereg* 원칙으로 유지한다.
9. **×12.7 ratio paired-bootstrap CI (§4.5 / §1.4 hardening).** Qwen3-VL Instruct vs Thinking correct-base df ratio의 paired-bootstrap CI 산출. 동일 stratified-sids 위 paired resample × ratio 재계산 (B=10,000), percentile CI. ~2 H100-hour. Round-1 MAJOR-6 / round-4 MAJ-7 요구사항. 본 결과 후 §1.4 / §4.5 / §8.2 의 "≥ 5× directional" 표현은 정량 CI로 대체된다.

항목 1–2는 가장 cheap한 rigor 향상, 항목 7 (재분석 only) 이 그 다음, 항목 9 + 8(b) (~2-5 H100-hour) + 항목 8(a) (~3 H100-hour per backbone) 가 다음, 항목 3은 가장 큰 GPU 부담을 요구하는 generalisation 항목이다.
...
**D.2.2 OneVision Main extension (llava-onevision-qwen2-7b-ov; n=200 stratified per dataset, B=2,000 bootstrap CI). Reproducibility source pointer 부록 §A.5.**
