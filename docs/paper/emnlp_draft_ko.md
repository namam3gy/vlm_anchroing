...
8. **Pre-registered single-cell hypothesis tests (§4.6 + §6.2.3 multiplicity hardening).** §4.6 14/84 Bonferroni-clean cells와 §6.2.3 Bonferroni-20을 cell-pre-registration 형태로 hardening — (a) §4.6에서 단일 (K, layer, statistic) cell을 사전 등록 후 단일 hypothesis test, (b) §6.2.3에서 27-cell grid selection을 별도 multiplicity layer로 포함하는 Bonferroni-540 conservative correction. Compute는 free — 기존 bootstrap raw data 위 분석 재실행. 본 paper의 directional findings를 confirmatory level로 hardening.
...
항목 1–2는 가장 cheap한 rigor 향상, 항목 7–8 (재분석 only) 가 그 다음, 항목 9 (~2 H100-hour) 가 다음, 항목 3은 가장 큰 GPU 부담을 요구하는 generalisation 항목이다.
