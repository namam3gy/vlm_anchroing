...
(1) VLM에서의 *independent-anchor open-numeric-estimation* anchoring을 식별·정량화하는 *first-evidence* 평가 프레임워크 (familiar-subject counting을 다루는 VLMBias [Vo, Nguyen et al., 2025]와는 cue source · 측정 대상이 상보적; §2). (2) gt-자유, baseline-relative direction-follow 표준 metric (`(pa − pb)·(anchor − pb) > 0 AND pa ≠ pb`). (3) 5 dataset × 6 model cross-dataset 증거. (4a) 5-model 메커니즘 panel에서 single-layer ablation이 5/5 null임을 보이고 그 *multi-layer redundancy*를 *attention pathway routing*의 속성으로 위치시키며, 그 routing의 결과가 *late residual stream에 통합*되어 single-layer projection으로 접근 가능해진다는 **routing vs integration framework**를 본 논문의 *이론적* 기여로 제시한다 — 이 framework는 (i) single-direction (LEACE/ActAdd) cross-dataset 실패의 사전 예측 + §6.4 LEACE rank-1 ChartQA +56 % 역행 *경험적 검증*의 **predict-then-verify chain (§5.2 → §6.4)**, (ii) E1d single-layer attention null과 E6 single-layer subspace projection의 양립성, (iii) late layer (L=26 / 28-layer) 선택의 정당화, (iv) projection (broad ablation 아닌) 도구 선택을 단일 mechanism-level 사슬로 묶는다 (§5.2 Insight 4 + §6.6 정리). **§4.6 γ-β residual-stream bridge가 본 framework의 두 번째 empirical anchor를 제공한다 — Qwen3-VL self-calibration K=1 subspace에서 late-stack (L=29-34) positive + mid-stack (L=20) negative 형태로 anchor 처리의 layer-routed structure를 14/84 cells Bonferroni-corrected (B=10,000) within-Thinking CI로 직접 관측 (자세히 §4.6).** (4b) Anchor-attention archetype이 *LM backbone이 아닌 encoder family*에 의해 결정됨 — 4 archetype (SigLIP-Gemma early / mid-stack cluster CLIP-ViT·ConvNeXt / Qwen-ViT late / FastVLM late text-stealing) 분리는 LLaMA-계열 LM을 공유하는 두 모델이 모두 mid-stack cluster에 속하는 falsifiable cross-cut. (5) Single-direction mitigation의 cross-dataset 실패를 *예측한 뒤 multi-direction subspace projection으로 우회*하며, 6-benchmark capability preservation까지 동시에 검증한다 (*free-lunch* — anchoring 효과 + 양 arm em + 일반 능력 보존 동시 충족; §6.2.3 / §6.5에서 형식 정의). **이 mitigation chain은 단일 모델 `llava-onevision-qwen2-7b-ov` 위에서의 case study이며**, cross-architecture 재calibration은 §8.2 한계로 명시된다. (6) γ-β reasoning pair (N=1 Qwen3-VL Instruct vs Thinking) × N=1 dataset (MathVista)에서 reasoning-amplifies-anchoring을 처음 보이는 *first-evidence* VLM 결과 — 이는 *hypothesis-generating existence proof*이며 cross-architecture · cross-dataset 검증은 후속 라운드 (§8.2).
...
**PlotQA single-dataset depth panel의 역할.** PlotQA는 본 paper main matrix의 **GT-range 가장 넓은 dataset** (정수 [1, 10000], 5-stratum sampled n=5,000)이며, §4.3 main matrix의 5-dataset breadth와 *상보적*인 *single-dataset depth* axis로 사용된다 — Phase-A H2 binary projection (§4.1 본 절) / (a − m) digit-pixel gap (§4.2 Table 3) / L1 6-bin gradient (§4.4) 의 *replication depth*가 PlotQA cell에서 selectively 가장 강하다. 5-dataset 중 anti-scaling (4B > 27B; §4.3 Insight 2) / em-positive baseline (em(a) > em(b) on 5/6 model; §4.2 Insight 3) / 가장 큰 (a-m) gap (§4.2 Table 3) 모두 PlotQA를 일차 evidence로 한다 — 즉 본 panel은 paper-wide pattern이 *가장 또렷이 분리되는* dataset에서 6-model breadth를 한 cell로 압축한다. (legacy VQAv2 number subset n=17,730 7-model panel은 §C.1 부록으로 이전 — 단일 자릿수 GT range [0,9] / skewed answer 분포로 인한 ceiling 한계 + S1 distance stratification 의미 약화 caveat 포함.)
...
![Figure 3 — Digit-pixel causality. Wrong-base × S1 paired conditional adopt. LLaVA-Interleave VQAv2에서 anchor=0.129 vs masked=0.068 (gap +6.1 pp). qwen2.5-vl-7b는 양 arm 모두 floor.](../figures/E5c_anchor_vs_masked_adopt.png)

**S1 confound resolution.** 본 비교는 wrong-base × S1 ([0,1] absolute on VQAv2/TallyQA, `max(1, 0.10·gt)` relative on chart-stack — 자세한 cutoff별 stratum 구성 + 거리 plausibility 감쇠 검증은 §C) 부분집합에서 수행된다. Wrong-base × S1에서 anchor가 gt에 plausibly 가까우면 "모델이 gt 쪽으로 복귀하는 것이 anchor 쪽으로 보이는" trivial 해석이 가능하다 — `(pa, pb, anchor)` 의 부호가 우연히 정렬될 수 있기 때문. (`a`, `m`) 페어는 *같은 anchor scene*에서 *디지트 픽셀만* 차이 — 거리·scene·plausibility는 동일하므로 이 confound는 두 arm에 *같은 정도로 작용*하고 (a − m) 차분에서 *상쇄*된다. 따라서 (a − m) gap > 0은 "S1에서 모델이 gt 회복" 가설로 설명 *불가* — 디지트 픽셀이 인과적이라는 *clean separation*이다.

**Table 3.** Wrong-base × S1 paired adoption + (a − m) gap. Bold = 큰 (a − m) gap.

| 데이터셋 | 모델 | adopt(a) | adopt(m) | (a − m) | 출처 |
|---|---|---:|---:|---:|---|
| VQAv2 | **llava-interleave** | 0.129 | 0.068 | **+6.1 pp** | E5c 3-model panel |
| VQAv2 | **gemma3-27b** | 0.138 | 0.082 | **+5.7 pp** | E5c 3-model panel |
| VQAv2 | qwen2.5-vl-7b | 0.070 | 0.066 | +0.4 pp | E5c 3-model panel |
| VQAv2 | llava-onevision *(Main)* | — | — | *(plan)* | OneVision E5c VQAv2 미실행 (plan §C.1) |
...
| InfoVQA | llava-onevision *(Main)* | 0.045 | 0.037 | +0.7 pp | E5b 5-strata × OneVision (§E.3) |
...
**Insight 2 (E7 6-model 일반화 + §6.2.3 forward link).** PlotQA + InfoVQA 6-model panel E7에서도 (a − m) gap이 6/6 모델 양 dataset에서 양수. Digit-pixel causality는 3-model 깊이 분석을 넘어 *모든 검출 가능 cell*에서 일반화된다. 이 (a − m) gap의 panel-wide 일관성은 §6.2.1의 SVD calibration이 (a − m) contrast를 사용하는 설계 선택을 행동 측에서 직접 정당화하며 — 만일 어떤 model/dataset에서 (a − m)이 부호 비일관이면 SVD가 잡는 principal direction이 그 cell에서 noise vector가 되었을 — §6.2.3 5-dataset paired-sids 표의 effect-size ordering (PlotQA largest, TallyQA floor)이 본 절 Insight 1의 "큰 끌림 → 큰 (a − m) gap" 단조성에 정렬됨을 사전 시사한다.

**Insight 3 (PlotQA un-mitigated free-lunch).** E7 panel의 *놀라움*: 6개 중 5개 모델이 PlotQA에서 `em(a) > em(b)` (em delta +0.6 ~ +5.0 pp; 출처 `docs/insights/E7-plotqa-infovqa-evidence.md` §3 PlotQA per-model em table). S1 cutoff가 anchor를 GT의 ±10 % 안에 두므로 anchor를 "그럴듯한 추측 단서"로 픽업하는 모델은 정확도를 *얻는다*. 이 패턴은 InfoVQA로 일반화하지 *않으며* (혼합 부호 — 같은 evidence file §4), *§6.2의 free-lunch mitigation이 이 baseline을 일반화 가능한 복구 메커니즘으로 변환하는 정확한 도메인*이다.
...
**Layer-specific sign-reversal — routing-vs-integration framework의 두 번째 empirical anchor.** within-Thinking paired (T_a − T_d per sid) bootstrap (B=10,000) 84-cell sweep에서 **14 / 84 cells가 Bonferroni-corrected (k=84) CI 0 제외** (Table 5):
...
### 5.1 Encoder family별 단일 peak + 4 archetype

5 mechanism panel 모델 × 200 stratified 자극의 (text → 두 번째 이미지) attention mass에서 각 모델은 *calibration dataset (VQAv2)에서 단일 peak layer*를 가진다. 4개 archetype이 분리된다: SigLIP-Gemma early (L5/42, gemma4-e4b), mid-stack cluster (L14-16, llava-1.5 / convllava), Qwen-ViT late (L22/28, qwen2.5-vl), FastVLM late + text-stealing (L22, fastvlm). FastVLM과 OneVision의 *cross-dataset peak variability*는 §5.3에서 별도 다룬다 (FastVLM: VQAv2 L22 / TallyQA L23 / InfoVQA L27 / PlotQA L17 — `_data/cross_dataset_peaks.csv`; OneVision: bimodal L14/L27).
...
**Insight 4 (Routing layer vs integration site).** Multi-layer redundancy는 *attention pathway*의 속성 — 즉 anchor 신호를 *routing*하는 site들이 redundant하다는 의미 — 이지 residual stream 표현 자체가 redundant라는 의미는 아니다. Routing이 multi-layer에 분산되어 있어도 그 결과는 점진적으로 residual에 *누적*된다. 충분히 후반 layer에 도달하면 분산되어 있던 multi-layer attention 기여가 잔차 표현에 *통합된 형태*로 자리 잡으며 single-layer site에서 접근 가능해진다. 이 *routing vs integration* 구분이 §6.2의 single-layer subspace projection이 §5.2 single-layer ablation null과 *양립하는 이유*이며, §6.6에서 본 framework의 세 paper 결정 (양립성 / late-layer 선택 / projection 선택)을 사전 예측한다.

본 framework는 두 *empirical anchor*를 가진다. **첫째**, §6.4 LEACE rank-1 ChartQA +56 % 역행이 *single-direction mitigation의 cross-dataset 실패*를 framework 예측으로부터 직접 검증 (predict-then-verify chain §5.2 → §6.4). **둘째**, §4.6 γ-β residual-stream bridge가 *layer-routed anchor 처리의 직접 관측*을 제공한다 — Qwen3-VL self-calibrated K=1 subspace에서 within-Thinking paired Δ가 mid-stack (L=20) negative + late-stack (L=29-34) positive sign-reversal로 14/84 Bonferroni-corrected cells에서 0 제외. Routing이 mid-stack에서 V_K subspace를 *suppress*하다가 late-stack에서 *integrate*하는 layer-dependent 경로가 잔차 amplitude 측정에서 직접 드러난다.
...
Upper-half ablation의 significant 결과 (§5.2)를 바탕으로 *soft re-weighting*: a-arm에서 두 번째 이미지 (anchor)에 대한 text → image attention weight를 강도 `s`로 곱한다 (`s` ∈ {0.0, 0.25, ..., 1.5}). Mid-stack cluster 2 모델 (LLaVA-1.5 / ConvLLaVA) Phase 2 full validation (17,730 base question × 5 cond, 모델당 88,650 records).

**Table 5.** E4 Phase 2 결과 (88,650 records / 모델, 출처 `docs/insights/E4-mitigation-evidence.md`). Bold = 열 단위 가장 큰 효과.
...
**Insight 1 (Free-lunch의 메커니즘적 의미).** acc(b) target-only 불변 + acc(d) neutral arm ±0.5 pp 안 + em(a) 양수 — hook은 single-image inference에 누출되지 않으며, 두 번째 이미지에 가독 digit이 없는 경우 *hook은 트리거되지만* 제거할 signal이 없다. 즉 *upper-half pathway가 모델 자체 답안 형성에 non-load-bearing*이며, 그 부담을 줄이면 anchor 영향만 제거된다. Table 5의 per-column bold는 이 분리를 그대로 반영한다 — LLaVA-1.5는 Δdf 측에서 가장 큰 효과 (−14.6 %), ConvLLaVA-7b는 Δem(a) 측에서 가장 큰 회복 (+1.30 pp).
...
§5.2 Insight 4의 routing-vs-integration framework는 본 절의 설계 선택을 사전 예측한다 — multi-layer attention routing을 모두 개입하는 것은 비현실적이지만, 그 routing의 결과가 누적되는 *residual integration site*에서 단일 layer 개입이 가능하다. §6.2는 이 framework의 구체화 — late residual layer L=26, K=8 부분공간, projection (ablation 아닌) — 이며, §6.6에서 framework 자체의 세 paper 결정 (single-layer 양립성 / late layer 선택 / projection 도구) 정당화를 정리한다.
...
**Table 7.** Multi-method 비교. Bold = 본 작업.
...
### 6.6 두 mitigation의 위치 — Routing vs Integration site

§6.1 (E4)와 §6.2 (E6)는 *상보적*이다. E4는 *mechanism* mitigation으로 attention pathway upper-half에서 *작동하고*, E6는 *representational* mitigation으로 residual stream에서 *작동한다*. 그러나 두 mitigation의 정합을 이해하려면 §5.2의 multi-layer redundancy 발견을 *어디에서* 적용하는가의 구분이 핵심이다.

**Multi-layer redundancy는 attention pathway의 속성이다 — residual stream에서도 그런 것은 아니다.** §5.2 E1d 결과는 어떤 단일 attention layer를 마스킹해도 다른 layer가 anchor 신호를 routing해 forward한다는 것이다. 이는 *routing layer*들이 redundant하다는 의미이다. 이 redundant routing의 결과로 anchor 정보는 점진적으로 **residual stream에 누적**된다. 충분히 후반 layer에 도달하면, 분산되어 있던 multi-layer attention 기여가 잔차 표현 안에 *통합된 형태*로 자리 잡는다. 이 통합 표현은 (1) low-dim이며 (§6.4 K=8 sweet spot의 mechanism-level 설명), (2) single-layer 단일 site에서 접근 가능하다. *Attention layer는 routing site, residual stream의 후반 layer는 integration site*인 것이다.

이 구분이 세 가지 paper 결정을 사전 예측한다.

1. **Single-layer attention ablation null과 single-layer subspace projection의 양립.** E1d는 *routing site*를 잘라본 결과이고 다른 routing layer가 부담을 받는다. E6는 *integration site*에서 누적 표현을 제거한다 — 누적 결과는 잘려나간 routing layer 하나 만으로 재구축되지 않는다. 두 결과는 모순이 아니며, 둘 다 multi-layer redundancy 모델의 직접 예측이다.

2. **Late layer 선택의 정당화.** Multi-layer redundant routing은 모든 attention layer에 동시 개입을 비현실적으로 만든다 (계산 비용 + normal computation 파괴). 단일 layer 개입이 강제되면 *어느* layer가 합당한가? 너무 이른 layer (예: L=5)는 통합이 아직 일어나지 않아 anchor 신호가 routing에 분산된 상태 — low-dim subspace로 압축되지 않으므로 K=8 projection이 잡아낼 게 없다. 너무 늦은 layer (decoding 직전 final output)는 답이 이미 결정되어 redirect 불가능하다. **L=26은 28-layer Qwen2 backbone의 *결정 직전 통합 site*** — anchor 표현이 통합 완료, final decoding 전. Pilot grid의 27-cell 탐색이 L=26을 winner로 산출한 것은 *empirical*이지만 그 winner가 왜 L=25 / L=27이 아니라 L=26인지의 mechanism-level 설명은 integration site framework에서 도출된다.

3. **Projection (not ablation)의 정당화.** Integration site에서 anchor 정보가 K=8로 압축되어 있다는 사실은 도구 선택에도 함의를 가진다. Ablation은 broad하게 정보를 0으로 만들지만 통합된 anchor 신호는 잔차의 K=8 부분공간이라는 *구조*에 자리 잡았다. Subspace projection은 정확히 그 K=8 차원만 제거하고 그 외 분산은 보존한다 — §7의 capability preservation (HallusionBench excludes zero, POPE pinned to zero on n=5,127)이 도구 선택의 정당성을 *외부 검증*한다.

본 framework는 `§5.2 (multi-layer redundancy in routing) → §6.2 (integration in late residual) → §6.4 (single-direction failure, multi-direction success at integration site) → §7 (capability preserved because intervention is structure-aware)`의 단일 chain을 형성하며, §1.3 / §8.1에서 본 논문의 *이론적* 기여로 framing되는 predict-then-verify 본체이다. 본 framework의 직접 falsifiable prediction — Thinking-mode trace 생성 동안 K=8 subspace amplitude가 자라야 한다 — 은 §8.4 후속 작업 1번으로 명시된다.

**Summary.** 본 논문이 §1.3 / 초록에서 *deployable mitigation* 으로 권장하는 것은 §6.2의 E6이다 — 단, **E6의 모든 검증 (§6.2 calibration · §6.2.3 5-dataset · §6.5 baseline 비교 · §7 6-benchmark capability preservation)은 단일 모델 `llava-onevision-qwen2-7b-ov` 위에서의 case study이다**. Cross-architecture 일반화 — SigLIP-Gemma early peak / Qwen-ViT late peak / FastVLM late text-stealing archetype 위에서의 (L*, K, α) 재calibration — 는 §8.2 한계로 명시되며, 본 논문이 검증한 *behavioral · mechanism* 결과는 multi-model panel (§4.1 6-model PlotQA, §4.3 5-dataset 6-model main matrix, §5.2 5-model 메커니즘 panel; legacy VQAv2 7-model panel은 §C.1 부록)에서, *deployable mitigation* 결과는 single-model case study에서 보고된다는 panel-scope 분리를 §1.3 · §1.5 (5) · §8.2와 일관 유지한다.
...
**Table 8.** E8 결과 (1.5h H200, no LLM judge). Bold = 통계적 유의성.
...
**4. Cross-architecture E6 replication (full close of CRIT-1).** §8.2 한계의 직접 close — SigLIP-Gemma early / Qwen-ViT late / FastVLM late text-stealing archetype 위 (L*, K, α) 재calibration. *후속 작업 1* eigenvalue spectrum이 양성이면 spectrum-predicts-cell 형태로 재구조화 가능 (brute-force grid search → spectral prediction). 추정 부담: ~30 H200-day for 3 archetypes (calibration · pilot grid · 5-dataset eval · capability eval).

**5. CAA · ITI 경험적 row (§8.2 deferred 항목 직접 close).** §6.5 Table 7 footnote의 구조적 reduction을 경험적 row로 보강. 추정 부담: ~3 H200-day. (§6.2.3 paired-bootstrap CI는 P1-3로 2026-05-10 본 라운드 직접 close — 본 항목에서 제외.)
...
**Caveat**. VQAv2 numeric subset은 *GT range가 단일 자릿수 [0,9]에 한정*되며 answer 분포도 *highly skewed* (anchor digit "2"가 ~30 %+ 차지). S1 distance stratification (`|a − GT| ≤ 1`) 의 의미가 chart-stack [1, 10000] 만큼 selective하지 않아 anchor stratum 분리력이 약하다. 따라서 본 panel은 §4.1 main 자리에서 §C.1로 이전되었으며, *cross-stimulus replication 증거*로만 인용된다. Anti-scaling (4B > 27B; §4.3 Insight 2) / em-positive baseline (em(a) > em(b); §4.2 Insight 3) / 가장 큰 (a-m) gap (§4.2 Table 3) 의 모든 paper-wide pattern은 본 legacy panel의 단일 자릿수 ceiling 한계로 *명확히 분리되지 않으며*, 이 점이 §4.1 main을 PlotQA로 옮긴 일차 동기이다.
...
**E5c (a, m) paired comparison이 이 confound를 직접 falsify한다.** 같은 wrong-base × S1 부분집합에서:
...
본문 §4.2 Table 3에서 wrong-base × S1 paired conditional adoption gap:

- LLaVA-Interleave-7b VQAv2: `adopt(a) = 0.129` vs `adopt(m) = 0.068` → **(a − m) = +6.1 pp**
- Gemma3-27b-it VQAv2: `adopt(a) = 0.138` vs `adopt(m) = 0.082` → **(a − m) = +5.7 pp**
- LLaVA-Interleave-7b TallyQA: 0.110 vs 0.084 → +2.5 pp
- Gemma3-27b-it TallyQA: 0.074 vs 0.053 → +2.1 pp

두 모델 × 두 dataset 모두에서 *디지트 픽셀이 있을 때만* effect가 발생, *디지트 픽셀을 제거하면* (a − m) gap이 양 — "S1에서 모델이 gt를 회복" 해석으로는 *(a − m) 차분이 0에 가까워야* 하지만 실제로 +5.7-6.1 pp 양수. 따라서 S1 peak는 *gt 회복 우연*이 아닌 *디지트 픽셀의 인과적 anchor pull* 에 기인한다.
