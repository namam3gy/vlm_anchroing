...
> **Status (2026-05-17):** Phase 0 완료, Phase 1–3 진행 예정. 본 subsection 은 paper Findings → Main tier conditional accept 의 핵심 조건 (`docs/paper/reviews/_final_summary.md` "Main-tier conditional on cross-architecture E6 instantiation"). Stage-4 paired-bootstrap table + 6-bench capability table 은 Phase 2/3 landing 후. **Phase 0 secondary finding**: L\*_qwen = 26 — OneVision L\* = 26 과 *동일* (둘 다 28-layer Qwen2.5-7B-derived LM backbone). 결과 evidence: `docs/insights/E6-cross-arch-qwen25vl-phase0.md`.

{{본 subsection 은 §6.1 calibrated subspace projection mitigation recipe 를 **Qwen2.5-VL-7B-Instruct** 위에 instantiate 한다. OneVision (SigLIP encoder + Qwen2 28-layer LM) 과 Qwen2.5-VL (Qwen2 ViT NaViT-style dynamic resolution + Qwen2 28-layer LM) 은 *encoder-family 가 다른* 두 architecture — *encoder-family transfer* 의 첫 측정. Recipe 는 §6.1 과 identical — (a − m) paired-inpaint calibration, 27-cell pilot grid topology (L ∈ {L*_qwen − 1, L*_qwen, L*_qwen + 1} × K ∈ {2, 4, 8} × α ∈ {0.5, 1.0, 2.0}; **L\*_qwen = 26** Phase 0 calibrate-subspace + SVD pool 후 `‖v_wrong[L]‖` top-1 으로 식별, evidence doc 위 단조 ramp 확인), Δem(a) ≥ −6pp deal-breaker + combined |Δdf(a)| ranking 으로 cell 선정 — *hyperparameter retuning 없이 동일 recipe portability 검증*. Design + budget detail 은 `docs/experiments/E6-cross-arch-design.md`.}}
