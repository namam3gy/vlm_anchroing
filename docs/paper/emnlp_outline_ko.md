...
본 논문은 각 question 위에 4-condition stimulus 를 구성한다. **b** (target only) 는 target 이미지만 단독으로 제시한 baseline; **a** (target + anchor) 는 target 옆에 *질문과 무관한 anchor digit 이미지* 를 함께 제시; **m** (target + masked) 는 a 와 *pixel-단위로 동일한 anchor 이미지에서 digit 만 Telea-inpainting* 으로 가린 변형; **d** (target + neutral) 는 target 옆에 *digit 이 전혀 없는 neutral 이미지* (FLUX render) 를 제시. 각 condition 이 isolate 하는 인과 통로: (a − b) 가 anchor 추가의 *총효과*, (a − m) 이 *digit pixel 단독 효과* (§3.2 에서 paired contrast 로 격상), (d − b) 가 *일반 distraction 통제*. 자극 예시 / prompt template / masked 이미지 생성은 Appendix A.
...
### 3.2 (a − m) paired contrast

*(a − m)* paired contrast 는 본 논문의 organizational backbone — phenomenon (§4 의 digit-pixel causal gate) 과 mitigation (§6 의 calibration 신호) 두 측면이 *동일한 자극 짝* 위에서 닫힌다. Substrate 가 작동하는 이유는 a 와 m 자극의 *pixel-단위 동일성*: 두 자극은 같은 anchor 이미지에서 출발하고 digit pixel 영역만 Telea-inpainting 으로 교체되므로 배경 / layout / 두-이미지 입력 부담이 모두 동일, (a − m) 차이는 *digit pixel 의 인과 효과만* 격리한다. 일반 distraction 은 별도로 d arm 이 통제하며, m ≈ d 는 falsifiable prediction 이다.

### 3.3 Metrics

본 paper 는 anchoring 을 base prediction (pb) 으로부터 anchor (z) 쪽으로의 *방향 이동* 으로 정의하고, primary metric 으로 *direction-follow* (DF) — c 의 prediction 이 anchor 방향으로 움직인 비율 (magnitude 무시) — 을 사용한다. 보조 metric 으로 *adopt* (literal copy 비율) 와 *exact-match* (정확도) 를 측정한다. 모든 metric 은 condition c ∈ {b, a, m, d} 마다 형식 동일.

**Metric definitions.** For each sample `i`, `gt_i` = ground truth, `z_i` = anchor digit value, `p_i^c` = parsed prediction under condition `c ∈ {b, a, m, d}`.

| Metric | Definition | Role |
|---|---|---|
| **Adopt_c** | `#(p^c == z AND pb != z) / #(pb != z) = P[p^c == z \| pb != z]` | Literal copy rate. |
| **DF_c** | `P[(p^c − pb)(z − pb) > eps \| \|z − pb\| > eps]` | Anchor 방향으로 prediction 이 움직인 비율 (magnitude-free). |
| **Exact-match_c** | `#(p^c == gt) / #(numeric pair)` | Accuracy (b-arm = baseline, a-arm = anchored). |

**Subsets:** `base-correct = { i | pb_i == gt_i }`, `base-wrong = { i | pb_i != gt_i }`.

### 3.4 Models and datasets

**Models.** LLaVA-OneVision-7B (Main), LLaVA-Interleave-7B, Qwen2.5-VL-7B/32B, Gemma3-4B/27B (6 open-weight). **Datasets.** TallyQA, ChartQA, PlotQA, InfoVQA, MathVista (5 numeric VQA). LLaVA-OneVision-7B 가 §5–§6 mechanism + mitigation 의 worked example. Spec / filter / anchor inventory 는 Appendix B / C.
...
{{본 subsection 은 6 model × 5 dataset main panel 위에서 anchoring 효과의 *전반적 크기* 와 *cross-panel robustness* 를 제시한다. 모델별 평균 direction-follow 약 5-28 %, adopt 약 2-19 % 의 range 가 모든 모델 × 모든 dataset cell 에서 양수 임을 main panel table 한 장으로 입증 — 효과의 질량이 literal copy 가 아닌 graded pull 에 있고, 특정 architecture / domain artifact 가 아님.}}
...
> Figure 1 의 두 panel 은 본문 `약 5-28 %` (df) / `약 2-19 %` (adopt) range 와 cell-by-cell 직접 대응한다. Per-cell 수치 (6 model × 5 dataset grid) 는 **Appendix D.1 / D.2** 참조 — D.1 / D.2 는 같은 metric 을 추가로 *base-wrong* subset 위에 병기한다.
...
{{본 subsection 은 §3.4 main 6-model panel (Gemma3-4b / 27b, Qwen2.5-VL-7b / 32b, LLaVA-Interleave-7b, LLaVA-OneVision-7b Main) 위에서 5 dataset (PlotQA, InfoVQA, ChartQA, MathVista, TallyQA) 의 *(text → 두 번째 이미지의 digit-pixel patches)* attention peak layer 를 식별한다. Per-model peak depth-norm (= peak_L / (n_layers−1)) 이 architecture 별로 — Gemma3-27b 0.46–0.48 ↔ Interleave-7b 1.00 (5/5 dataset) — 약 30 layer 스케일로 시프트하여 *single uniform causal site 부재*. **Intra-model 은 dataset 사이에 stable** — 4/6 model (Gemma3-4b L=17, Qwen2.5-VL-7b L=16, Interleave-7b L=31, OneVision Main L=14) 이 5/5 dataset 동일 peak; Gemma3-27b 는 4/5 동일 (L=29) + InfoVQA 만 1-layer 차이 (L=28); Qwen2.5-VL-32b 는 4/5 동일 (L=42) + TallyQA 만 late-stack outlier (L=62). 즉 peak heterogeneity 는 **cross-model 축에 집중** 하고 intra-model cross-dataset 축은 거의 없음 — integration site 가 *architecture-specific* 이면서 *input distribution (dataset) 에 invariant* 라는 §5.3 routing-and-integration framework 의 직접 supporting evidence (model 별 calibration 의 dataset-level reusability 도 함께 시사). Model-specific peak 은 §5.2 single-layer ablation 의 target 으로 사용. 측정은 `extract_attention_mass.py --bbox-file inputs/irrelevant_number_bboxes.json` 으로 anchor image 의 digit-pixel patches 만 골라 layer-별 attention mass 산출; sample pool 은 OneVision Main 의 dataset 별 susceptibility CSV top-decile-500 + bottom-decile-500 (max n=1000, dataset 별 cap 적용).}}
...
**Preview Figure §5.2a — K=1/2/4/8 progression heatmap (PlotQA × OneVision Main, L ∈ {14, 20, 22, 26}, α ∈ {0.5, 1, 2}, E6 calibration pilot).**
<img src="../figures/paper_5_2a_E6_pilot_grid_plotqa.png" alt="K progression heatmap" width="700"/>

> 4 metric × 4 layer × 3 α × 4 K. L=26, α=1.0 에서 K=1 → K=2 → K=4 → K=8 progression: Δadopt(a) +0.5 → +0.0 → −2.4 → **−6.5** pp (PlotQA, n=250). K=1 / K=2 모두 near-zero, K=4 mid, K=8 strong — K-monotonic improvement 가 multi-direction representation 의 직접 evidence. 같은 cell 의 보조 metric: Δem(a) −1.2 → −0.8 → 0.0 → **+3.6 pp** (target 정확도도 개선), Δem(b) +1.2 → +1.6 → +2.0 → **+4.8 pp**. ★ 표시 cell (L=26, α=1.0, K=8) 이 §6 selected. 전체 5-dataset K sweep 은 §5.2b figure (`paper_5_2b_layer_sweep_delta_df.png`) 와 cross-check.

**Preview Figure §5.2b — K=8 vs K=1 across 4 layers × 5 datasets (P4 verification).**
<img src="../figures/paper_5_2b_layer_sweep_delta_df.png" alt="K=8 vs K=1 layer sweep" width="700"/>

> Solid line = K=8, dashed = K=1, across L ∈ {14, 20, 22, 26} on 5 datasets. K=8 가 K=1 보다 anchoring 감소 효과 강함 — 대표적으로 (i) PlotQA L=26 K=8 **−5.07** [−6.90, −3.30] vs K=1 −0.39 [−1.60, +0.87] (~13× stronger), (ii) TallyQA L=26 K=1 **+1.37 [+0.54, +2.19] backfire** (sign-clean positive) vs K=8 −0.34 null — single-direction 으로는 부족 + 일부 dataset 에서는 backfire 까지 일으킨다는 5-dataset cross-check.

> **Reproducibility.** 위 §5.2a + §5.2b figure 및 모든 per-cell Δ 수치는 `notebooks/paper_section_5_2_subspace_sweep.ipynb` 가 single + prefetch (batch=1, prefetch_workers=16) 으로 OneVision Main 위에서 재실행 가능. Canonical CSV: `outputs/paper/section_5_e6_steering/_data/E6_pilot_grid.csv` (48 cells) + `_data/p4_layer_sweep_per_cell_ci.csv` (40 cells). 본문 인용 수치는 bf16 precision 내 paper §6.2.2 Table 6.2 와 1:1 일치.
...
{{본 subsection 은 §5.1 의 *cross-model peak heterogeneity + intra-model cross-dataset 안정* + §5.2 의 *single-layer ablation null* + *K-subspace sweep* 의 mechanism finding 을 *routing-and-integration* framework 로 합성한다 — anchoring 신호가 multi-layer attention pathway 를 통해 *routing* 되고 (multi-layer 처리, model 마다 다른 site 를 통과), residual stream 의 한 model-specific site 에 *integration* 으로 통합됨. *Cross-model heterogeneity* 와 *single-layer null* 이 framework 의 자연 prediction — uniform causal site 가 없는 것은 routing pathway 가 multi-layer 인 직접 결과, integration site 가 architecture-specific 인 것도 같은 해석에 부합. **Intra-model cross-dataset 안정** 은 framework 의 *dataset-invariance* 를 추가 지지 — 같은 model 안에서 5 dataset 의 peak layer 가 일치 (4/6 model 5/5 + 나머지 2/6 model 4/5) 하므로 integration site 가 input distribution 에 둔감하고 model 의 *structural property* 임을 시사 — 이로써 §6 의 (a − m) calibration (특정 calibration distribution 위에서 추출한 subspace) 이 5 dataset 전체로 일반화되는 결과의 mechanistic 정당화도 함께 제공된다. OneVision Main 의 경우 integration site 는 §6.2.4 sweep 위 L=26 (28-layer Qwen2 의 0.96 depth-norm, 즉 late residual). Anchoring representation 의 전체 구조에 대한 post-hoc characterization 으로 §5.1 / §5.2 finding 을 묶는 통합 narrative.}}
...
| TallyQA | test | deterministic 5,000-cap of 38,245 eligible from full split (38,589); first 5,000 question_ids in loader order | numeric GT, range [0, 8] | **5,000** | 0–8 (counting) | strongly skewed small (1–4 dominant) |
...
> **두 cohort 병기.** D.1 / D.2 는 동일 metric (df(a), adopt(a)) 을 두 cohort 위에서 병기하며 각 cell 은 **`broad / base-wrong`** 두 값을 나란히 적는다 — **broad** 는 cell 의 *모든* sample (`base-correct ∪ base-wrong`) 위 rate 로 본문 §4.1 의 graded-pull range 에 직접 대응한다. **base-wrong** 은 b-arm 예측이 GT 와 다른 sample 만 위 rate 로 canonical aggregator (notebook §11 — `paper_cross_model_cross_dataset.ipynb`) 가 `docs/insights/_data/main_panel_5dataset_per_cell.csv` 에 기록하는 값과 일치한다. DF / adopt 둘 다 eps=0 form (denominator: `pb ≠ z` 이고 anchor present; H1 prompt rerun + 2026-05-18 canonical 기준). 두 cohort 의 cell-by-cell 재현은 같은 notebook §11–§13 참조.
...
|---|---|---|---|---|---|---:|
| OneVision-7B (Main) | 3.9 / 11.2 | 6.1 / 17.8 | 12.2 / 23.5 | 9.9 / 18.7 | 11.6 / 22.0 | **8.7 / 18.6** |
| Interleave-7B | 5.1 / 11.7 | 15.6 / 20.2 | 16.5 / 21.2 | 24.9 / 27.0 | 23.5 / 25.8 | **17.1 / 21.2** |
| Qwen2.5-VL-7B | 4.4 / 14.1 | 3.1 / 11.7 | 9.6 / 21.4 | 4.3 / 19.9 | 4.8 / 17.9 | **5.2 / 17.0** |
| Qwen2.5-VL-32B | 3.7 / 12.0 | 4.1 / 18.0 | 8.9 / 22.4 | 3.3 / 12.8 | 4.9 / 17.9 | **5.0 / 16.6** |
| Gemma3-4B | 11.8 / 20.8 | 21.5 / 37.9 | 41.9 / 48.5 | 35.1 / 46.9 | 28.8 / 39.1 | **27.8 / 38.6** |
| Gemma3-27B | 8.7 / 18.5 | 11.8 / 29.6 | 26.9 / 40.8 | 15.4 / 30.3 | 19.9 / 36.4 | **16.5 / 31.1** |

> Per-model mean range — broad: **5.0 – 27.8**; base-wrong: **16.6 – 38.6** (denominator 가 base-wrong 으로 좁아진 데 따른 magnitude shift; cell-wise 부호는 두 cohort 모두 30/30 cell 양수로 일관).
...
| Model | TallyQA | ChartQA | MathVista | PlotQA | InfoVQA | **Mean** |
|---|---|---|---|---|---|---:|
| OneVision-7B (Main) | 1.1 / 4.0 | 1.7 / 3.6 | 6.5 / 8.8 | 5.0 / 7.6 | 2.1 / 2.0 | **3.3 / 5.2** |
| Interleave-7B | 1.6 / 4.1 | 2.8 / 2.0 | 10.2 / 11.1 | 7.3 / 7.6 | 5.8 / 5.8 | **5.6 / 6.1** |
| Qwen2.5-VL-7B | 0.9 / 2.6 | 0.9 / 2.3 | 4.6 / 6.1 | 2.0 / 6.2 | 1.4 / 3.2 | **2.0 / 4.1** |
| Qwen2.5-VL-32B | 1.2 / 3.4 | 0.8 / 1.8 | 6.8 / 12.4 | 1.2 / 2.3 | 2.9 / 9.4 | **2.6 / 5.9** |
| Gemma3-4B | 6.3 / 11.9 | 11.0 / 16.6 | 42.0 / 43.8 | 22.0 / 25.1 | 13.3 / 15.8 | **18.9 / 22.6** |
| Gemma3-27B | 4.3 / 9.9 | 3.7 / 7.8 | 25.0 / 32.8 | 8.9 / 13.7 | 11.0 / 17.2 | **10.6 / 16.3** |

> Per-model mean range — broad: **2.0 – 18.9**; base-wrong: **4.1 – 22.6** (denominator 가 base-wrong 으로 좁아진 데 따른 magnitude shift).
