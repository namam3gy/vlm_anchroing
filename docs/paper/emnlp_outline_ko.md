...
> **Status (2026-05-17):** 실험 진행 중. 본 subsection 은 paper Findings → Main tier conditional accept 의 핵심 조건 (`docs/paper/reviews/_final_summary.md` "Main-tier conditional on cross-architecture E6 instantiation"). 결과 landing 후 Stage-4 paired-bootstrap table + 6-bench capability table 추가 + §8 "Mitigation scope" prose 동시 update.

{{본 subsection 은 §6.1 calibrated subspace projection mitigation recipe 를 **Qwen2.5-VL-7B-Instruct** 위에 instantiate 한다. OneVision (SigLIP encoder + Qwen2 28-layer LM) 과 Qwen2.5-VL (Qwen2 ViT NaViT-style dynamic resolution + Qwen2 28-layer LM) 은 *encoder-family 가 다른* 두 architecture — *encoder-family transfer* 의 첫 측정. Recipe 는 §6.1 과 identical — (a − m) paired-inpaint calibration, 27-cell pilot grid topology (L ∈ {L*_qwen − 1, L*_qwen, L*_qwen + 1} × K ∈ {2, 4, 8} × α ∈ {0.5, 1.0, 2.0}; L*_qwen 은 PlotQA n=200 attention probe 로 식별), Δem(a) ≥ −6pp deal-breaker + combined |Δdf(a)| ranking 으로 cell 선정 — *hyperparameter retuning 없이 동일 recipe portability 검증*. Design + budget detail 은 `docs/experiments/E6-cross-arch-design.md`.}}
