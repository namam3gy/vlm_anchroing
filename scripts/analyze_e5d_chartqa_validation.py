"""E5d — ChartQA distance-validation analysis (relative-cutoff scheme).

Loads the latest stratified run under
outputs/experiment_e5d_chartqa_validation/<model>/, computes per-stratum
...
- docs/insights/_data/E5d_chartqa_per_stratum.csv  — per-(stratum, base) table
- docs/figures/E5d_chartqa_decay.png               — decay curve (correct/wrong)
...
EXPERIMENT_DIR = "experiment_e5d_chartqa_validation"
...
def plot_decay(summary: pd.DataFrame, out_path: Path) -> None:
    ...
    ax.set_title("E5d — ChartQA distance validation (n=200, llava-interleave-7b)")
    ax.grid(alpha=0.3)
    ax.legend()
    fig.tight_layout()
...
def run(model: str = "llava-next-interleaved-7b") -> dict:
    ...
    out_csv = PROJECT_ROOT / "docs" / "insights" / "_data" / "E5d_chartqa_per_stratum.csv"
    out_csv.parent.mkdir(parents=True, exist_ok=True)
    summary.to_csv(out_csv, index=False)

    fig_dir = PROJECT_ROOT / "docs" / "figures"
    fig_dir.mkdir(parents=True, exist_ok=True)
    out_png = fig_dir / "E5d_chartqa_decay.png"
